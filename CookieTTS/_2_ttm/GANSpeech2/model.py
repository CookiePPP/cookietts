...
class Aligner(nn.Module):
    ...
    def forward(self,# gt_frame_logf0s,# FloatTensor[B,     4, mel_T]
        ...
        out['pred_mel_a']   = pred_mel  # [B, n_mel, mel_T]
...
class Generator(nn.Module):
    def __init__(self, h):
        ...
        self.durgenerator = FFT(h.dur_hidden_dim, h.dur_n_heads, h.dur_ff_dim, h.dur_n_layers, ff_kernel_size=h.dur_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim,              output_dim=3)# dur + f0s + voiced
        self.f0sgenerator = FFT(h.f0s_hidden_dim, h.f0s_n_heads, h.f0s_ff_dim, h.f0s_n_layers, ff_kernel_size=h.f0s_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim+3,            output_dim=2*self.n_f0s)
        self.melgenerator = FFT(h.mel_hidden_dim, h.mel_n_heads, h.mel_ff_dim, h.mel_n_layers, ff_kernel_size=h.mel_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim+3+self.n_f0s, output_dim=h.n_mel_channels)
    
    def forward(self, gt_char_logf0,# FloatTensor[B, mel_T]
                     gt_char_voiced,# FloatTensor[B, mel_T]
                    gt_frame_logf0s,# FloatTensor[B,     4, mel_T]
        ...
        # DurF0s Generator
        pred_durlogf0voiced = self.durgenerator(encoder_outputs, text_lengths)# [B, txt_T, mem_dim] -> [B, 3, txt_T]
        pred_dur, pred_char_logf0, pred_char_voiced = pred_durlogf0voiced.transpose(1, 2).chunk(3, dim=1)# -> [B, 1, txt_T], [B, 1, txt_T], [B, 1, txt_T]
        pred_char_voiced = pred_char_voiced.sigmoid()*get_mask_from_lengths(text_lengths).unsqueeze(1)
        out['pred_dur'        ] = pred_dur        # [B, 1, txt_T]
        out['pred_char_logf0' ] = pred_char_logf0 # [B, 1, txt_T]
        out['pred_char_voiced'] = pred_char_voiced# [B, 1, txt_T]
        out['pred_durlogf0voiced'] = torch.cat((pred_dur, pred_char_logf0, pred_char_voiced), dim=1)# [B, 3, txt_T]
        ...
        # F0s Generator
        gt_dur = hard_alignments.sum(dim=1)
        gt_durlogf0voiced = torch.stack((gt_dur, gt_char_logf0, gt_char_voiced), dim=1)# [B, 3, txt_T]
        gt_durlogf0voiced = gt_durlogf0voiced @ hard_alignments.transpose(1, 2)# [B, 3, txt_T] @ [B, txt_T, mel_T] -> [B, 3, mel_T]
        
        f0sgen_input = torch.cat((attention_contexts, gt_durlogf0voiced), dim=1)# -> [B, mem_dim+f0s_dim, mel_T]
        pred_logf0svoiced = self.f0sgenerator(f0sgen_input.transpose(1, 2), mel_lengths)# [B, mem_dim+f0s_dim, mel_T] -> [B, mel_T, 2*f0s_dim]
        pred_logf0s, pred_voiced = pred_logf0svoiced.transpose(1, 2).chunk(2, dim=1)# -> [B, f0s_dim, mel_T], [B, f0s_dim, mel_T]
        pred_voiced = pred_voiced.sigmoid()*get_mask_from_lengths(mel_lengths).unsqueeze(1)
        out['pred_logf0s'] = pred_logf0s# [B, f0s_dim, mel_T]
        out['pred_voiced'] = pred_voiced# [B, f0s_dim, mel_T]
        
        # Mel Generator
        melgen_input = torch.cat((attention_contexts, gt_durlogf0voiced, gt_frame_logf0s), dim=1)# -> [B, mem_dim+f0s_dim, mel_T]
        pred_mel = self.melgenerator(melgen_input.transpose(1, 2), mel_lengths).transpose(1, 2)# [B, mem_dim+f0s_dim, mel_T] -> [B, n_mel, mel_T]
    ...
    def infer(self, text, text_lengths, torchmoji_hdn, speaker_id, speaker_f0_meanstd, speaker_slyps_meanstd, hard_alignments=None, sigma=1.0, randn_max=99.9, max_decode_length=1024):
        ...
        pred_dur = F.relu(pred_dur.squeeze(1)).round()# -> [B, txt_T]
        mel_lengths = pred_dur.sum(1).long()# -> LongTensor[B]
        out['mel_lengths'] = mel_lengths
        
        pred_durlogf0voiced = torch.cat((pred_dur.unsqueeze(1), pred_char_logf0*(pred_char_voiced>0.01), pred_char_voiced), dim=1)# [B, 3, txt_T]
        ...
        if hard_alignments is not None:
            ...
            attention_contexts = self.get_attention_from_lengths(encoder_outputs.transpose(1, 2), pred_dur, text_lengths).transpose(1, 2)# -> [B, mem_dim, mel_T]
        
        # F0s Generator
        if hard_alignments is not None:
            pred_durlogf0voiced = pred_durlogf0voiced @ hard_alignments.transpose(1, 2)# [B, 3, txt_T] @ [B, txt_T, mel_T] -> [B, 3, mel_T]
        else:
            pred_durlogf0voiced = self.get_attention_from_lengths(pred_durlogf0voiced.transpose(1, 2), pred_dur, text_lengths).transpose(1, 2)# -> [B, 3, mel_T]
        
        f0sgen_input = torch.cat((attention_contexts, pred_durlogf0voiced), dim=1)# -> [B, mem_dim+f0s_dim, mel_T]
        pred_logf0svoiced = self.f0sgenerator(f0sgen_input.transpose(1, 2), mel_lengths)# [B, mem_dim+f0s_dim, mel_T] -> [B, mel_T, 2*f0s_dim]
        pred_logf0s, pred_voiced = pred_logf0svoiced.transpose(1, 2).chunk(2, dim=1)# -> [B, f0s_dim, mel_T], [B, f0s_dim, mel_T]
        pred_voiced = pred_voiced.sigmoid()*get_mask_from_lengths(mel_lengths).unsqueeze(1)
        pred_logf0s = pred_logf0s*pred_voiced.round()# [B, f0s_dim, mel_T]
        
        # Mel Generator
        melgen_input = torch.cat((attention_contexts, pred_durlogf0voiced, pred_logf0s), dim=1)# -> [B, mem_dim+f0s_dim, mel_T]
        pred_mel = self.melgenerator(melgen_input.transpose(1, 2), mel_lengths).transpose(1, 2)# [B, mem_dim+f0s_dim, mel_T] -> [B, n_mel, mel_T]
        out['pred_mel'] = pred_mel# [B, n_mel, mel_T]
...
class Discriminator(nn.Module):
    def __init__(self, h):
        ...
        self.f0sdiscriminator = FFT(h.d_f0s_hidden_dim, h.d_f0s_n_heads, h.d_f0s_ff_dim, h.d_f0s_n_layers, ff_kernel_size=h.d_f0s_ff_kernel_size, n_pre_layers=getattr(h, 'd_f0s_n_pre_layers', 0), add_position_encoding=True, input_dim=h.mem_dim+self.n_f0s+3, output_dim=1)
    ...
    def discriminator_loss(self, gt, pred_encoder_outputs, pred_durlogf0voiced, pred_f0s, pred_mel, hard_alignments):
        ((pred_durlogf0voiced_realness, gt_durlogf0voiced_realness),
         (           pred_f0s_realness,            gt_f0s_realness),
         (           pred_mel_realness,            gt_mel_realness)) = self(gt, pred_encoder_outputs, pred_durlogf0voiced, pred_f0s, pred_mel, hard_alignments)
        
        dur_loss = discriminator_loss(pred_durlogf0voiced_realness, gt_durlogf0voiced_realness, gt['text_lengths'])
        f0s_loss = discriminator_loss(           pred_f0s_realness,            gt_f0s_realness,  gt['mel_lengths'])
        mel_loss = discriminator_loss(           pred_mel_realness,            gt_mel_realness,  gt['mel_lengths'])
        
        return dur_loss, f0s_loss, mel_loss, (
         (pred_durlogf0voiced_realness, gt_durlogf0voiced_realness),
         (           pred_f0s_realness,            gt_f0s_realness),
         (           pred_mel_realness,            gt_mel_realness)
        )
    
    def forward(self, gt, pred_encoder_outputs, pred_durlogf0voiced, pred_f0s, pred_mel, hard_alignments):
        ...
        pred_durlogf0voiced = torch.stack((
                pred_durlogf0voiced[:, 0, :]+pred_durlogf0voiced[:, 0, :].new_empty(pred_durlogf0voiced[:, 0, :].shape).uniform_(-0.5, 0.5),
                pred_durlogf0voiced[:, 1, :]+pred_durlogf0voiced[:, 1, :].new_empty(pred_durlogf0voiced[:, 1, :].shape).normal_(std=2.5),
                pred_durlogf0voiced[:, 2, :]+pred_durlogf0voiced[:, 2, :].new_empty(pred_durlogf0voiced[:, 2, :].shape).normal_(std=2.5)), dim=1)# [B, txt_T], [B, txt_T], [B, txt_T] -> [B, 3, txt_T]
        # mask dur tensor
        ...
        #pr_c_dmask = (pred_durlogf0voiced[:, 0:1] < 0.50) & (pred_durlogf0voiced[:, 0:1] >= 0.0)
        #pr_c_vmask = (pred_durlogf0voiced[:, 2:3] < 0.01) & (pred_durlogf0voiced[:, 2:3] >= 0.0)
        ...
        #pred_durlogf0voiced = pred_durlogf0voiced.masked_fill(padded_mask, 0.0)# [B, 3, txt_T]
        ...
        pred_durlogf0voiced_realness = self.durdiscriminator(torch.cat((pred_durlogf0voiced, encoder_outputs.transpose(1, 2)), dim=1).transpose(1, 2), gt['text_lengths'])
        ...
        if False:
            encoder_outputs = encoder_outputs + (pred_encoder_outputs*self.encoder_weight)
        encoder_outputs = encoder_outputs.transpose(1, 2)# [B, txt_T, mem_dim] -> [B, mem_dim, txt_T]
        
        attention_contexts      =   encoder_outputs @ hard_alignments.transpose(1, 2)# [B, mem_dim, txt_T] @ [B, txt_T, mel_T] -> [B, mem_dim, mel_T]
        gt_frame_durlogf0voiced = gt_durlogf0voiced @ hard_alignments.transpose(1, 2)# [B,       3, txt_T] @ [B, txt_T, mel_T] -> [B,       3, mel_T]
        
        pred_f0s_realness = self.f0sdiscriminator(torch.cat((pred_f0s, gt_frame_durlogf0voiced, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        gt_f0s_realness   = self.f0sdiscriminator(torch.cat((  gt_f0s, gt_frame_durlogf0voiced, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        
        pred_mel_decoded = self.meldecoder((pred_mel+mel_noise).transpose(1, 2), gt['mel_lengths']).transpose(1, 2)
        gt_mel_decoded   = self.meldecoder((  gt_mel+mel_noise).transpose(1, 2), gt['mel_lengths']).transpose(1, 2)
        
        pred_mel_realness = self.meldiscriminator(torch.cat((pred_mel_decoded, gt_f0s, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        ...
        if self.out_tanh:
            pred_durlogf0voiced_realness, gt_durlogf0voiced_realness = pred_durlogf0voiced_realness.tanh()*0.5, gt_durlogf0voiced_realness.tanh()*0.5
            pred_f0s_realness, gt_f0s_realness = pred_f0s_realness.tanh()*0.5, gt_f0s_realness.tanh()*0.5
            pred_mel_realness, gt_mel_realness = pred_mel_realness.tanh()*0.5, gt_mel_realness.tanh()*0.5
        
        return ((pred_durlogf0voiced_realness, gt_durlogf0voiced_realness),
                (           pred_f0s_realness,            gt_f0s_realness),
                (           pred_mel_realness,            gt_mel_realness))
...
class Model(nn.Module):
    ...
    def generator_loss(self, gt, pred_encoder_outputs, pred_durf0s, pred_f0s, pred_mel, hard_alignments):
        ((pred_durlogf0voiced_realness, gt_durlogf0voiced_realness),
         (           pred_f0s_realness,            gt_f0s_realness),
         (           pred_mel_realness,            gt_mel_realness)) = self.discriminator(gt, pred_encoder_outputs, pred_durf0s, pred_f0s, pred_mel, hard_alignments)
         
        dur_loss = generator_loss(pred_durlogf0voiced_realness, gt['text_lengths'])
        f0s_loss = generator_loss(           pred_f0s_realness,  gt['mel_lengths'])
        mel_loss = generator_loss(           pred_mel_realness,  gt['mel_lengths'])
    ...
    def forward(self, gt_frame_logf0s,# FloatTensor[B,     4, mel_T]
        ...
        out_tmp = self.generator(gt_char_logf0, gt_char_voiced, gt_frame_logf0s, gt_frame_voiceds, gt_mel, mel_lengths, text, text_lengths, speaker_id, speaker_f0_meanstd, speaker_slyps_meanstd, gt_sylps, torchmoji_hdn, freq_grad_scalar, alignment)
        out = {**out, **out_tmp}
