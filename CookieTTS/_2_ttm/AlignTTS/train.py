...
from model import AlignTTS, load_model
...
from loss_function import AlignTTSLoss
from logger import AlignTTSLogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = AlignTTSLogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def validate(hparams, args, file_losses, loss_scalars, model, criterion, valset, best_val_loss_dict, iteration,
    ...
    if args.rank == 0:
        ...
        if iteration > 1:
            log_terms = (loss_dict_total, best_val_loss_dict, model, y, y_pred, align, iteration)
...
def train(args, rank, group_name, hparams):
    ...
    print("Initializing AlignTTS Logger.")
