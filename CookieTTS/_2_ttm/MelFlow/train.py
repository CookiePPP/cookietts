...
from model import MelFlow, load_model
...
from loss_function import MelFlowLoss
from logger import MelFlowLogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = MelFlowLogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def save_checkpoint(model, optimizer, learning_rate, iteration, hparams, best_validation_loss, average_loss,
    ...
    tqdm.write("Saving model and optimizer state at iteration {} to {}".format(iteration, filepath))
...
def validate(hparams, args, file_losses, loss_scalars, model, criterion, valset, best_val_loss_dict, iteration,
             collate_fn, logger, save_alignments):
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(val_loader), desc="Validation", total=len(val_loader), smoothing=0): # i = index, batch = stuff in array[i]
            ...
            with torch.random.fork_rng(devices=[0,]):
                ...
                y_pred = force(model, valid_kwargs=model_args, **{**y, 'align_with_z': not model.use_flow_conds,})
            
            loss_dict, file_losses_batch, align = criterion(model, y_pred, y, loss_scalars,)# calc_alignments=True, save_alignments=save_alignments)
        ...
        # infer with model to get more plotting data
        #with torch.random.fork_rng(devices=[0,]):
            #torch.random.manual_seed(i)# use repeatable seeds during validation so results are more consistent and comparable.
        inv_y_pred = model.infer(y['text'],
                         y['text_lengths'],
                           y['speaker_id'],
                        y['torchmoji_hdn'],
                       y_pred['alignment'],
...
def train(args, rank, group_name, hparams):
    ...
    print("Initializing MelFlow Loss func.")
    criterion = MelFlowLoss(hparams)
    
    print("Initializing MelFlow Logger.")
    logger = prepare_directories_and_logger(hparams, args)
    
    # Load checkpoint if one exists
    best_validation_loss = 1e9# used to see when "best_val_model" should be saved
        ...
        try:
            for epoch in tqdm(range(epoch_offset, hparams.epochs), initial=epoch_offset, total=hparams.epochs, desc="Epoch:", position=1, unit="epoch"):
                ...
                for i, batch in tqdm(enumerate(train_loader), desc="Iter:  ", smoothing=0, total=len(train_loader), position=0, unit="iter"):
                    ...
                    y_pred = force(model, valid_kwargs=model_args, **{**y, 'align_with_z': align_with_z, 'mdn_align_grads': mdn_align_grads,})
                    
                    loss_scalars = {
                   'melglow_total_loss_weight': melglow_total_loss_weight,
                             'mdn_loss_weight': mdn_loss_weight,
                             'dur_loss_weight': dur_loss_weight,
                           'melenc_kld_weight': melenc_kld_weight,
                    }
                    ...
                    if iteration%checkpoint_interval==0 or os.path.exists(save_file_check_path):
                        ...
                        if rank == 0:
                            ...
                            save_checkpoint(model, optimizer, learning_rate, iteration, hparams, best_validation_loss, average_loss, best_val_loss_dict, best_loss_dict, speaker_lookup, speakerlist, checkpoint_path)
                    ...
                    if iteration%validation_interval==0 or os.path.exists(save_file_check_path):
                        ...
                        val_loss, best_val_loss_dict, file_losses = validate(hparams, args, file_losses, loss_scalars, model, criterion, valset, best_val_loss_dict, iteration, collate_fn, logger, save_alignments)
                        file_losses = write_dict_to_file(file_losses, os.path.join(args.output_directory, 'file_losses.csv'), args.n_gpus, rank)
                        if use_scheduler:
                        ...
                        if (val_loss < best_validation_loss):
                            ...
                            if rank == 0 and hparams.save_best_val_model:
                                ...
                                save_checkpoint(
                                    model, optimizer, learning_rate, iteration, hparams, best_validation_loss,
