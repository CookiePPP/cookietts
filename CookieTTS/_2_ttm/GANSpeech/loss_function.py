...
class Loss(nn.Module):
    ...
    def d_forward(self, iteration, model, pr, gt, loss_scalars, loss_params, file_losses=None):
        ...
        if self.GAN_enable:
            if self.fast_discriminator_mode:
                ...
                mel_loss = discriminator_loss(pr['pr_mel_realness'], pr['gt_mel_realness'], gt[ 'mel_lengths'])
            else:
                dtype = next(model.parameters()).dtype
                _ = model.discriminator.discriminator_loss(gt, pr['encoder_outputs'].detach().to(dtype),
                                                           pr['pr_logdur'].detach().to(dtype),
                                                           pr['pr_logcf0'].detach().to(dtype),
                                                           pr['pr_mel'].detach().to(dtype),
                                                           alignment.detach())
                dur_loss, cf0_loss, mel_loss, (
    ...
    def g_forward(self, iteration, model, pr, gt, loss_scalars, loss_params, file_losses=None, save_alignments=False):
        ...
        if 'pr_mel_a' in pr:# Spectrogram Loss
            mask = get_mask_from_lengths(gt['mel_lengths']).unsqueeze(1)
            pr_mel = pr['pr_mel_a'].float()
            gt_mel = gt['gt_mel'  ].float()
            numel = gt['mel_lengths'].sum()*gt_mel.shape[1]
            
            loss_dict['ga_mel_MAE'] = F. l1_loss(pr_mel, gt_mel, reduction='sum')/numel
            loss_dict['ga_mel_MSE'] = F.mse_loss(pr_mel, gt_mel, reduction='sum')/numel
        
        if 'pr_gate' in pr:# Gate Loss
            gt_gate = gt['gt_gate_logits'][:, :pr['pr_gate'].shape[1]]# [B, mel_T]
            gt_gate[:, -1] = (gt_gate.sum(dim=1) == 0.0).float()          # [B, mel_T][:, -1] = [B]
            
            gt_gate = gt_gate      .masked_select(mask[:, 0, :pr['pr_gate'].shape[1]])# [B*mel_T]
            pr_gate = pr['pr_gate'].masked_select(mask[:, 0, :pr['pr_gate'].shape[1]])# [B*mel_T]
            loss_dict['ga_gate_BCE'] = nn.BCEWithLogitsLoss(pos_weight=self.pos_weight)(pr_gate, gt_gate)
            del gt_gate, pr_gate
        
        if 'soft_alignments' in pr:# Diagonal Alignment Loss
            dec_lengths = (gt['mel_lengths']//self.n_frames_per_step).clamp(max=pr['soft_alignments'].shape[1])
            not_truncated  = ~(gt['pres_prev_state'] | gt['cont_next_iter'])
            loss_dict['ga_att_loss'] = self.guided_att(pr['soft_alignments'][not_truncated], gt['text_lengths'][not_truncated], dec_lengths[not_truncated])
        
        if 'soft_alignments' in pr and loss_scalars['ga_hard_att_MSE_weight'] > 0.0 or loss_scalars['ga_hard_att_MSE_weight'] > 0.0:# Hard Attention Loss
            dec_lengths = (gt['mel_lengths']//self.n_frames_per_step).clamp(max=pr['soft_alignments'].shape[1])
            ...
            loss_dict['ga_hard_att_MSE'] = (F.mse_loss(pr['soft_alignments'], pr['hard_alignments'], reduction='none')*att_mask).sum()/gt['mel_lengths'].sum()
            loss_dict['ga_hard_att_MAE'] = (F. l1_loss(pr['soft_alignments'], pr['hard_alignments'], reduction='none')*att_mask).sum()/gt['mel_lengths'].sum()
        
        if 'soft_alignments' in pr:# Attention Metrics (AvgMaxAttention, Diagonality, MaxDuration, ...)
            alignments = pr['soft_alignments']
        ...
        if 'pr_logdur' in pr:# Char Duration Loss
            ...
            pr_dur = pr['pr_logdur']# [B, txt_T]
            gt_char_dur = alignment.detach().sum(dim=1).clamp(min=0.4).log()# [B, mel_T, txt_T] -> [B, txt_T]
            loss_dict['g_dur_MAE'] = (F. l1_loss(pr_dur, gt_char_dur, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
            loss_dict['g_dur_MSE'] = (F.mse_loss(pr_dur, gt_char_dur, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
        
        if 'pr_logcf0' in pr:# Char Pitch Loss
            logf0_mask = gt['gt_char_voiced']>0.01
            text_mask = get_mask_from_lengths(gt['text_lengths'])
            pr_logf0 = pr['pr_logcf0'    ]# [B, txt_T]
            gt_logf0 = gt['gt_char_logf0']# [B, txt_T]
            loss_dict['g_logf0_MAE'] = (F. l1_loss(pr_logf0, gt_logf0, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
            loss_dict['g_logf0_MSE'] = (F.mse_loss(pr_logf0, gt_logf0, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
        
        if 'pr_char_voiced' in pr:# Char Voiced Loss
            text_mask = get_mask_from_lengths(gt['text_lengths'])
            pr_voiced = pr['pr_char_voiced'].float()# [B, txt_T]
            gt_voiced = gt['gt_char_voiced'].float()
            loss_dict['g_voiced_MAE'] = (            F. l1_loss(pr_voiced, gt_voiced, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
            loss_dict['g_voiced_MSE'] = (            F.mse_loss(pr_voiced, gt_voiced, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
            loss_dict['g_voiced_BCE'] = (F.binary_cross_entropy(pr_voiced, gt_voiced, reduction='none')*text_mask).sum(dtype=torch.float)/text_mask.sum()
        
        if 'pr_mel' in pr:# Mel Loss
            mel_mask = get_mask_from_lengths(gt['mel_lengths']).unsqueeze(1)
            loss_dict['g_mel_MAE'] = (F. l1_loss(pr['pr_mel'], gt['gt_mel'], reduction='none')*mel_mask).sum(dtype=torch.float)/(mel_mask.sum()*gt['gt_mel'].shape[1])
            loss_dict['g_mel_MSE'] = (F.mse_loss(pr['pr_mel'], gt['gt_mel'], reduction='none')*mel_mask).sum(dtype=torch.float)/(mel_mask.sum()*gt['gt_mel'].shape[1])
        ...
        if self.GAN_enable:
            ...
            args = (gt, pr['encoder_outputs'].detach().to(dtype),
                        pr['pr_logdur'].to(dtype),
                        pr['pr_logcf0'].to(dtype),
            ...
            loss_dict['g_class_cf0'] = cf0_loss
            loss_dict['g_class_mel'] = mel_loss
            if self.fast_discriminator_mode:
                pr['pr_dur_realness'], pr['gt_dur_realness'] = pr_dur_realness, gt_dur_realness
                pr['pr_cf0_realness'], pr['gt_cf0_realness'] = pr_cf0_realness, gt_cf0_realness
