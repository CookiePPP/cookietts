...
class Logger(SummaryWriter):
    ...
    def log_validation(self, reduced_loss_dict, reduced_bestval_loss_dict, model, y, y_pred, iteration, prepend='validation'):
        ...
        gt_char_dur = alignment.detach().sum(dim=1).clamp(min=0.4).log()
        ...
        for idx in range(n_items):# plot target spectrogram of longest audio file(s)
            ...
            if 'pr_mel' in y_pred:
                self.add_image(f"{prepend}_{idx}/mel_pred",
                    plot_spectrogram_to_numpy(y_pred['pr_mel'][idx, :, :mel_len].data.float().cpu().numpy(), range=mag_range),
                    iteration, dataformats='HWC')
            
            if 'pr_mel_a' in y_pred:
                self.add_image(f"{prepend}_{idx}/mel_pred_ga",
                    plot_spectrogram_to_numpy(y_pred['pr_mel_a'][idx, :, :mel_len].data.float().cpu().numpy(), range=mag_range),
                    iteration, dataformats='HWC')
            ...
            # Pred Char Durations
            if 'pr_logdur' in y_pred:
                pred_dur = y_pred['pr_logdur'][idx, :txt_len].data.cpu()# -> [T]
            ...
            # Pred Char Log F0
            if 'pr_logcf0' in y_pred:
                pred_f0s = y_pred[    'pr_logcf0'][idx].data.cpu()# -> [T]
                gt_f0s   =      y['gt_char_logf0'][idx].data.cpu()# -> [T]
                #pred_f0s[y_pred['pr_logcf0'][idx, 0, :txt_len].data.cpu()<=0.01] = float('nan')
            ...
            if 'pr_char_voiced' in y_pred:
                pred_vo = y_pred['pr_char_voiced'][idx, 0, :txt_len].data.cpu()# -> [T]
            ...
            # Pred Frame Log F0s
            if 'pr_logf0s' in y_pred:
                pred_f0s = y_pred['pr_logf0s'      ][idx, 1, :mel_len].data.cpu()# -> [T]
            ...
            if 'pr_voiced' in y_pred:
                pred_vo = y_pred['pr_voiced'       ][idx, 1, :mel_len].data.cpu()# -> [T]
