...
def validate(h, args, file_losses, model, criterion, valset, loss_scalars, best_val_loss_dict, iteration, collate_fn, logger, inference_mode=False):
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(val_loader), desc="Validation", total=len(val_loader), smoothing=0): # i = index, batch = stuff in array[i]
            ...
            loss_dict_g, file_losses_batch_g = criterion.g_forward(iteration, model, y_pred, y, loss_scalars, {}, save_alignments=save_alignments)
...
def train(args, rank, group_name, h):
    ...
    print('Initializing VDGANSpeech...')
    model = load_model(h)
    print('Done')
    global model_args
    ...
    if True:# define optimizer (any params without requires_grad are ignored)
        #optimizer = apexopt.FusedAdam(filter(lambda p: p.requires_grad, itertools.chain(model.melgenerator.parameters(), model.durgenerator.parameters())), lr=learning_rate, weight_decay=h.weight_decay)
    ...
    print("Initializing VDGANSpeech Loss func.")
    criterion = Loss(h)
    criterion.cuda()
    
    print("Initializing VDGANSpeech Logger.")
        ...
        try:
            for epoch in tqdm(range(epoch_offset, h.epochs), initial=epoch_offset, total=h.epochs, desc="Epoch:", position=1, unit="epoch"):
                ...
                for i, batch in tqdm(enumerate(train_loader), desc="Iter:  ", smoothing=0, total=len(train_loader), position=0, unit="iter"):
                    ...
                    loss_scalars = {
                    'ga_mel_MAE_weight':               ga_mel_MAE_weight,
                    'ga_mel_MSE_weight':               ga_mel_MSE_weight,
                    'ga_gate_BCE_weight':              ga_gate_BCE_weight,
                    'ga_att_loss_weight':              ga_att_loss_weight,
                    'ga_hard_att_MSE_weight':          ga_hard_att_MSE_weight,
                    'ga_hard_att_MAE_weight':          ga_hard_att_MAE_weight,
                    'ga_att_diagonality_weight':       0.0,#ga_att_diagonality_weight,
                    'ga_att_top1_avg_prob_weight':     0.0,#ga_att_top1_avg_prob_weight,
                    'ga_att_top2_avg_prob_weight':     0.0,#ga_att_top2_avg_prob_weight,
                    'ga_att_top3_avg_prob_weight':     0.0,#ga_att_top3_avg_prob_weight,
                    'ga_att_avg_max_dur_weight':       0.0,#ga_att_avg_max_dur_weight,
                    'ga_att_avg_min_dur_weight':       0.0,#ga_att_avg_min_dur_weight,
                    'ga_att_avg_avg_dur_weight':       0.0,#ga_att_avg_avg_dur_weight,
                    'ga_att_avg_missing_enc_p_weight': 0.0,#ga_att_avg_missing_enc_p_weight,
                    'ga_att_avg_attscore_weight':      0.0,#ga_att_avg_attscore_weight,
                    'g_dur_MAE_weight'     : g_dur_MAE_weight,
                    'g_dur_MSE_weight'     : g_dur_MSE_weight,
                    'g_logf0_MAE_weight'   : g_logf0_MAE_weight,
                    'g_logf0_MSE_weight'   : g_logf0_MSE_weight,
                    #'g_voiced_MAE_weight'  : g_voiced_MAE_weight,
                    #'g_voiced_MSE_weight'  : g_voiced_MSE_weight,
                    #'g_voiced_BCE_weight'  : g_voiced_BCE_weight,
                    #'g_logf0s_MAE_weight'  : g_logf0s_MAE_weight,
                    #'g_logf0s_MSE_weight'  : g_logf0s_MSE_weight,
                    #'g_voiceds_MAE_weight' : g_voiceds_MAE_weight,
                    #'g_voiceds_MSE_weight' : g_voiceds_MSE_weight,
                    #'g_voiceds_BCE_weight' : g_voiceds_BCE_weight,
                    'g_mel_MAE_weight'     : g_mel_MAE_weight,
                    'g_mel_MSE_weight'     : g_mel_MSE_weight,
                    'g_class_dur_weight'   : g_class_dur_weight,
                    'g_class_cf0_weight'   : g_class_cf0_weight,
                    'g_class_mel_weight'   : g_class_mel_weight,
                    'd_class_dur_weight'   : d_class_dur_weight,
                    'd_class_cf0_weight'   : d_class_cf0_weight,
                    'd_class_mel_weight'   : d_class_mel_weight,
                    }
                    loss_params = {
                    'g_class_dur_active_thresh': g_class_dur_active_thresh,
                    'g_class_f0s_active_thresh': g_class_f0s_active_thresh,
                    'g_class_mel_active_thresh': g_class_mel_active_thresh,
                    'd_class_dur_active_thresh': d_class_dur_active_thresh,
                    'd_class_f0s_active_thresh': d_class_f0s_active_thresh,
                    'd_class_mel_active_thresh': d_class_mel_active_thresh,
                    }
                    ...
                    if h.GAN_enable:
                        model.discriminator.cf0_noise = cf0_noise_std
                    if h.fast_discriminator_mode:
                        # Update Generator
                        optimizer.zero_grad(set_to_none=True)# clear/zero the gradients from the last iter
                        loss_dict_g, file_losses_batch_g = criterion.g_forward(iteration, model, y_pred, y, loss_scalars, loss_params, file_losses, save_alignments=save_alignments)
                    ...
                    if (iteration > 1e3 and (reduced_loss > LossExplosionThreshold)):
