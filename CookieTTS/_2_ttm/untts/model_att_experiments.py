...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None, attention_override=None, t_max=None, t_min=None):
        ...
        B_mul_n_heads, dec_T, enc_T = attention_scores.shape
        attention_scores = attention_scores.view(B_mul_n_heads//self.head_num, self.head_num, dec_T, enc_T)
...
class LenPredictorAttention(nn.Module):
    ...
    def forward(self, encoder_outputs, melenc_outputs, encoder_lengths, output_lengths, cond_lens=None, attention_override=None):
...
class GMMAttention(nn.Module): # Experimental from NTT123
    ...
    #def gmm_jit(self, input, memory, pos, delta_min_limit: float, delta_offset: float):
        ...
        delta_ = F.softplus(delta_)       # [B, 1, n_mix] Remove ability to move backwards in the text
    ...
    def forward_(self, encoder_outputs, melenc_outputs, encoder_lengths, pos, loc, mask=None):# [B, seq_len, dim], int, [B]
        ...
        next_attention_context = attention_weights[:,:,:-1] @ encoder_outputs[:,1:] # [B, dec_T, enc_T-1] @ [B, enc_T-1, att_dim] -> [B, dec_T, att_dim]
        #next_attention_context = attention_weights[:,:,:-2] @ encoder_outputs[:,2:] # [B, dec_T, enc_T-2] @ [B, enc_T-2, att_dim] -> [B, dec_T, att_dim]
    ...
    def forward(self, encoder_outputs, melenc_outputs, encoder_lengths, output_lengths, cond_lens=None, attention_override=None):# [B, seq_len, dim], int, [B]
        """
        PARAMS
        ------
        attention_hidden_state: attention rnn last output
        ...
        if attention_override is None:
            ...
            pos = torch.arange(enc_T, device=encoder_outputs.device, dtype=encoder_outputs.dtype)[None, :, None]# [1, enc_T, 1]
            ...
            attention_weights = attention_override
            attention_context = attention_weights @ encoder_outputs# [B, dec_T, enc_T] @ [B, enc_T, att_dim] -> [B, dec_T, att_dim]
...
class PositionalAttention(nn.Module):
    def __init__(self, hparams):
        ...
        self.mha_attention = MultiHeadAttention(self.pos_enc_dim, self.head_num, bias=True)
    ...
    def forward(self, encoder_outputs, melenc_outputs, encoder_lengths, output_lengths, cond_lens=None, attention_override=None):# [B, seq_len, dim], int, [B]
        ...
        if hasattr(self, 'enc_positional_embedding') or hasattr(self, 'pos_embedding_kv'):# (optional) add position encoding to Encoder outputs
            ...
            if hasattr(self, 'pos_embedding_kv'):
                ...
                if self.training:# randomly pick between gt and predicted frames per char (during inference both values are the same and this will have no effect)
                    gt_frames_per_char = output_lengths.to(pred_output_lengths)/cond_lens# [B]/[B] -> [B]
        ...
        #######################################################################
        if hasattr(self, 't_min') and self.training:# apply random temperature to Key values (which will be used for Attention inside MHA)
            T = torch.rand((k.shape[0], dec_T_max+1, 1), device=k.device, dtype=k.dtype) * (self.t_max-self.t_min) + self.t_min # [B, 1+dec_T, 1]
            #k = k/T[:,0:1]# [B, enc_T, enc_dim] / [B, 1, enc_dim]
            q = q/T[:,1:] # [B, dec_T, att_dim] / [B, dec_T, enc_dim]
        
        attn_mask = get_mask_3d(output_lengths, cond_lens) if (cond_lens is not None) else None#[B, dec_T, enc_T]
        
        output, attention_scores = self.mha_attention(q, k, v, mask=attn_mask, attention_override=attention_override)
...
class Decoder(nn.Module):
    def __init__(self, hparams):
        ...
        self.use_dur_pred_for_inference = hparams.use_duration_predictor_for_inference
        if self.use_dur_pred_for_inference:
            self.dur_pred_attention = LenPredictorAttention(hparams)
    ...
    def infer(self, encoder_outputs, melenc_outputs, encoder_lengths, output_lengths, cond_lens=None, speaker_ids=None, sigma=None, att_override=None):
        """ Decoder inference
        PARAMS
        ------
        ...
        if hasattr(self, 'dur_pred_attention'): # (optional) use Duration Predictor for Attention (used when trained with MelEnc)
            _, att_override = self.dur_pred_attention(encoder_outputs, melenc_outputs, encoder_lengths, output_lengths, cond_lens=cond_lens)
        
        cond, attention_scores = self.positional_attention(encoder_outputs, melenc_outputs, encoder_lengths, output_lengths, cond_lens=cond_lens, attention_override=att_override)
        cond = cond.transpose(1, 2)# [B, enc_T, enc_dim] -> [B, enc_dim, dec_T] # Masked Multi-head Attention
...
class UnTTS(nn.Module):
    def __init__(self, hparams):
        ...
        self.melenc_drop_rate = hparams.melenc_drop_rate
    ...
    def forward(self, inputs):
        ...
        melenc_outputs = self.mel_encoder(gt_mels, output_lengths, speaker_ids=speaker_ids) if (self.mel_encoder is not None and self.melenc_drop_rate <= random.random()) else None# [B, dec_T, melenc_dim]
