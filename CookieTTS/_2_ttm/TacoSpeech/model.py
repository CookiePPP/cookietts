...
class VariancePredictor(nn.Module):
    """
    Predicts Duration, Pitch, Voiced, Spectral Tilt using FFT Encoder outputs and VAE Latent.
    """
    def __init__(self, hparams, mem_dim):
        ...
        for i in range(self.lstm_n_layers):
            self.lstm_cell.append(LSTMCellWithZoneout(input_dim, self.lstm_dim, dropout=self.dropout, zoneout=self.zoneout))
            input_dim = self.lstm_dim
        self.lstm_cell = nn.ModuleList(self.lstm_cell)
        self.lstm_proj = LinearNorm(input_dim, 8)
        
        self.mem_cell = LSTMCellWithZoneout(mem_dim, mem_dim, dropout=0.0, zoneout=0.5)
        
        self.state_pred = []
        for i in range(self.lstm_n_layers):
            lin = ResBlock1d(mem_dim+(2*self.lstm_dim if i else 0), 4*self.lstm_dim, n_layers=3, n_dim=mem_dim+256, n_blocks=1, kernel_w=1, dropout=0.15)
    ...
    def get_memory_summary(self, memory, text_lengths):# list[FloatTensor[B, mem_dim]], FloatTensor[B]
        txt_T = len(memory)
        B, mem_dim = memory[0].shape
        
        # Predict Final LSTM States with Memory
        mem_states       = (memory[0].new_zeros(B, self.lstm_dim), memory[0].new_zeros(B, self.lstm_dim))
        mem_final_states = [memory[0].new_zeros(B, self.lstm_dim).requires_grad_(), memory[0].new_zeros(B, self.lstm_dim).requires_grad_()]
        ...
        memory_summary = mem_final_states[0]
        return memory_summary
    
    def forward(self, memory, fesvd, text_lengths):# [B, txt_T, mem_dim], [B, 8, txt_T], [B]
        B, txt_T, mem_dim = memory.shape
        
        # Predict FESVD with Memory + Teacher-Forcing
        memory = memory.unbind(1)# [B, txt_T, mem_dim] -> [[B, mem_dim],]*txt_T
        fesvd  = F.pad(fesvd, (1, -1)).unbind(2)# [B, 8, txt_T] -> [[B, 8],]*txt_T
        
        states       = [(memory[0].new_zeros(B, self.lstm_dim), memory[0].new_zeros(B, self.lstm_dim)) for i in range(self.lstm_n_layers)]
        final_states = [[memory[0].new_zeros(B, self.lstm_dim),
                         memory[0].new_zeros(B, self.lstm_dim)] for i in range(self.lstm_n_layers)]
        
        pred_fesvd = []
        for i, (x, c) in enumerate(zip(memory, fesvd)):
            final_idx = (text_lengths==i+1)
            final_idx_sum = final_idx.sum()
            
            x = torch.cat((x, c), dim=1)# [B, mem_dim], [B, 8] -> [B, mem_dim+8]
            ...
            pred_fesvd.append(self.lstm_proj(x))# .append([B, 8])
        pred_fesvd = torch.stack(pred_fesvd, dim=2)# [B, 8, txt_T]
        pred_fesvd = pred_fesvd*get_mask_from_lengths(text_lengths).unsqueeze(1)
        
        memory_summary = self.get_memory_summary(memory, text_lengths)
        
        shifted_final_states = [(None, None),]+final_states
        shifted_final_states = shifted_final_states[:-1]
        h_c_mulogvar = []
        for i in range(self.lstm_n_layers):
            h, c = shifted_final_states[i]
            if h is not None and c is not None:
                input = torch.cat((memory_summary, h, c), dim=1)# [B, mem_dim + 2*lstm_dim]
            else:
                input = memory_summary
            h_c_mulogvar.append(self.state_pred[i](input))# [B, 4*lstm_dim]
        h_c_mulogvar = torch.stack(h_c_mulogvar, dim=1)# [B, n_lstm, 4*lstm_dim]
        ...
        return pred_fesvd, h_c_mulogvar, final_states# [B, 8, txt_T], [B, n_lstm, 4*lstm_dim]
    
    def infer(self, memory, text_lengths, std=None):# [B, txt_T, mem_dim]
        B, txt_T, mem_dim = memory.shape
        if std is not None:
            self.std = std
        
        # Predict FESVD with Memory + Autoregressive LSTM stack
        memory = memory.unbind(1)# [B, txt_T, mem_dim] -> [[B, mem_dim],]*txt_T
        
        # summarise memory sequence
        memory_summary = self.get_memory_summary(memory, text_lengths)
        ...
        for i in range(self.lstm_n_layers):
            ...
            h_c_mu_logvar = self.state_pred[i](input)# [B, 4*lstm_dim]
            h_c_mu, h_c_logvar = h_c_mu_logvar.chunk(2, dim=1)# [B, 4*lstm_dim] -> [B, 2*lstm_dim], [B, 2*lstm_dim]
            h_c = self.reparameterize(h_c_mu, h_c_logvar)# [B, 2*lstm_dim], [B, 2*lstm_dim] -> [B, 2*lstm_dim]
            #h_c.normal_(std=1.0)
            h, c = h_c.chunk(2, dim=1)# [B, 2*lstm_dim] -> [B, lstm_dim], [B, lstm_dim]
            states.append((h, c,))
        
        c = memory[0].new_zeros(B, 8)# [B, 8]
        pred_fesvd = []
        for i, x in enumerate(memory):
            x = torch.cat((x, c), dim=1)# [B, mem_dim], [B, 8] -> [B, mem_dim+8]
            for j, lstm_cell in enumerate(self.lstm_cell):
                ...
                x = states[j][0]
            c = self.lstm_proj(x)
            pred_fesvd.append(c)# .append([B, 8])
        pred_fesvd = torch.stack(pred_fesvd, dim=2)# [B, 8, txt_T]
        pred_fesvd = pred_fesvd*get_mask_from_lengths(text_lengths).unsqueeze(1)
...
class Decoder(nn.Module):# Takes Text + Attention and outputs audio/spectrogram.
    def __init__(self, hparams, mem_dim, output_dim=None):
        ...
        self.mel_proj = LinearNorm(mem_dim, output_dim or hparams.n_mel_channels)
    ...
    def forward(self, attention_contexts, mel_lengths):# [B, mel_T, mem_dim]
    ...
    def infer(self, attention_contexts, mel_lengths):
        decoder_outputs, pred_mel = self.fft_blocks(attention_contexts, mel_lengths)
...
class TacoSpeech(nn.Module):# Main module, contains all the submodules for the network.
    def __init__(self, hparams):
        super(TacoSpeech, self).__init__()
        ...
        if True:# Variance Predictor
            self.varpred = VariancePredictor(hparams, mem_dim)
        ...
        if True:# Conditional Scale Shift the memory with FESV+logdur
            self.cond_ss = SSSUnit(mem_dim, self.fesvd_dim)
        
        if True:
            self.ffesv_bn = MaskedBatchNorm1d(7, eval_only_momentum=False, affine=False, momentum=0.1)
            self.fvarpred = Decoder(hparams, mem_dim, 7)
            self.fcond_ss = SSSUnit(mem_dim, 7)
    ...
    def forward(self, gt_audio, audio_lengths,# FloatTensor[B, wav_T],  LongTensor[B]
                        gt_mel,   mel_lengths,# FloatTensor[B, n_mel, mel_T],  LongTensor[B]
                          text,  text_lengths,#  LongTensor[B, txt_T],  LongTensor[B]
                              ...
                              gt_fesv        ,# FloatTensor[B, 7, mel_T]
                              gt_frame_voiced,# FloatTensor[B, mel_T]
        ...
        if hasattr(self, 'varpred'):
            pred_fesvd, h_c_mulogvar, h_c = self.maybe_cp(self.varpred, *(memory, bn_fesvd, text_lengths))# [B, 8, txt_T], [B, n_lstm, 4*lstm_dim]
            outputs['varpred_pred_fesvd'] = pred_fesvd# [B, 8, txt_T]
            outputs['varpred_hidden_pred'] = h_c_mulogvar# [B, n_lstm, 4*lstm_dim]
            outputs['varpred_hidden_gt'  ] = h_c         # [B, n_lstm, 4*lstm_dim]
            del h_c_mulogvar, h_c
        
        # Char Conditional Scale Shift
        memory = self.cond_ss(memory, bn_fesvd.transpose(1, 2))# [B, txt_T, mem_dim], [B, 8, txt_T] -> [B, txt_T, mem_dim]
        
        # Extend memory -> attention_contexts
        attention_contexts = alignment @ memory# [B, mel_T, txt_T] @ [B, txt_T, mem_dim] -> [B, mel_T, mem_dim]
        
        # (Frame FESV Decoder)
        mel_mask = get_mask_from_lengths(mel_lengths)# [B, mel_T]
        bngt_fesv = self.ffesv_bn(gt_fesv, mel_mask)
        outputs['bngt_fesv'] = bngt_fesv
        _, pred_frame_fesv, _ = self.fvarpred(attention_contexts, mel_lengths)
        pred_frame_fesv[:, 0, :] = pred_frame_fesv[:, 0, :].round()
        pred_frame_fesv[:, 1, :].masked_fill_(pred_frame_fesv[:, 0, :]==0., 0.0)
        outputs['pred_frame_fesv'] = pred_frame_fesv# [B, mel_T, 7]
        
        # Frame Conditional Scale Shift
        attention_contexts = self.fcond_ss(attention_contexts, bngt_fesv.transpose(1,2))# [B, mel_T, mem_dim]
        
        # (Decoder) attention_contexts -> spectrogram
        decoder_outputs, pred_mel, aux_out = self.decoder(attention_contexts, mel_lengths)# [B, mel_T, mem_dim] -> [B, mel_T, mem_dim], [B, mel_T, n_mel_channels]
        outputs['decoder_outputs'] = decoder_outputs         # [B, mel_T, mem_dim]
        outputs['pred_mel']        = pred_mel.transpose(1, 2)# [B, mel_T,   n_mel] -> [B, n_mel, mel_T]
        
        if self.HiFiGAN_enable:
            outputs['hifigan_inputs'] = decoder_outputs.transpose(1, 2)# [B, mem_dim, mel_T]
    ...
    def inference(self, text_seq, text_lengths, speaker_id, torchmoji_hdn,
                    res_sigma=1.0, mel_z_sigma=1.0,
                    bn_fesvd=None, char_dur=None, gt_mel=None, alignment=None,
                    mel_lengths=None, trim_frames=0, trim_chars=4, disable_glow_decoder=False):# [B, enc_T], [B], [B], [B], [B, tm_dim]
        outputs = {}
        ...
        if hasattr(self, 'varpred'):
            bn_fesvd = self.varpred.infer(memory, text_lengths, std=res_sigma)# [B, 8, txt_T], [B, n_lstm, 4*lstm_dim]
        
        if char_dur is None:
            bn_char_logdur = bn_fesvd[:, 0:1, :]# [B, 1, txt_T]
            text_mask = get_mask_from_lengths(text_lengths)# [B, txt_T]
            char_logdur = self.ldur_bn.inverse(bn_char_logdur, text_mask)# -> [B, 1, txt_T]
        ...
        if mel_lengths is None:
            mel_lengths = char_dur.sum(dim=1).long()# [B]
        ...
        # Frame Variance Predictor
        pred_frame_fesv = self.fvarpred.infer(attention_contexts, mel_lengths)[1]# [B, 7, mel_T]
        
        # Frame Conditional Scale Shift
        attention_contexts = self.fcond_ss(attention_contexts, pred_frame_fesv.transpose(1,2))# [B, mel_T, mem_dim]
        
        # (Decoder/Attention) memory -> pred_mel
        decoder_outputs, pred_mel = self.decoder.infer(attention_contexts, mel_lengths)
