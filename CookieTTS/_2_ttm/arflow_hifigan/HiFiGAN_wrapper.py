...
class HiFiGAN(nn.Module):
    def __init__(self, hparams=None):
        ...
        if hparams is None:
            return
        assert hparams.batch_size>=hparams.HiFiGAN_batch_size, 'HiFiGAN batch size must be even or greater than ARFlow_HiFiGAN batch size!'
    ...
    def forward(self, pred:dict, gt:dict, reduced_loss_dict:dict, loss_dict:dict, loss_scalars:dict):
        ...
        if pred['hifigan_indexes'].sum():
            ...
            pred_mel   = self.STFT.mel_spectrogram_with_grad(pred_audio.squeeze(1))
