...
from model import ARFlow_HiFiGAN, load_model
...
from loss_function import ARFlow_HiFiGANLoss
from logger import ARFlow_HiFiGANLogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = ARFlow_HiFiGANLogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def validate(hparams, args, file_losses, model, criterion, hifiGAN, valset, loss_scalars, best_val_loss_dict, iteration, collate_fn, logger):
    ...
    with torch.no_grad():
        ...
        inv_y_pred = None
        if model.decoder.dec_glow:
            inv_y_pred = model.inference(y['text'],
                             y['text_lengths'],
                               y['speaker_id'],
                            y['torchmoji_hdn'],
                   bn_fesvd=y_pred['bn_fesvd'],
             alignment=y_pred['mdn_alignment'],
                  mel_lengths=y['mel_lengths'],
                              mel_z_sigma=0.95,
                             char_z_sigma=0.95,
                              trim_frames=  0,)
            
            #bn_fesvd_MAE = torch.nn.L1Loss(inv_y_pred['pred_bn_fesvd'], y_pred['bn_fesvd'])
            #reduced_loss_dict['bn_fesvd_MAE'] = reduce_tensor(bn_fesvd_MAE.data, args.n_gpus).item() if hparams.distributed_run else bn_fesvd_MAE.item()
...
def train(args, rank, group_name, hparams):
    ...
    print('Initializing ARFlow_HiFiGAN...')
    ...
    print("Initializing ARFlow_HiFiGAN Loss func.")
    criterion = ARFlow_HiFiGANLoss(hparams)
    
    print("Initializing ARFlow_HiFiGAN Logger.")
    ...
    while training:
        try:
            for epoch in tqdm(range(epoch_offset, hparams.epochs), initial=epoch_offset, total=hparams.epochs, desc="Epoch:", position=1, unit="epoch"):
               ...
               "melflow_total_loss_weight": melflow_total_loss_weight,
              "charglow_total_loss_weight": charglow_total_loss_weight,
