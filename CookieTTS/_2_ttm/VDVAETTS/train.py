...
from model import VDVAETTS, load_model
...
from loss_function import VDVAETTSLoss
from logger import VDVAETTSLogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = VDVAETTSLogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def validate(hparams, args, file_losses, model, criterion, hifiGAN, valset, loss_scalars, best_val_loss_dict, iteration, collate_fn, logger):
    ...
    with torch.no_grad():
        ...
        with torch.random.fork_rng(devices=[0,]):
            torch.random.manual_seed(i)# use repeatable seeds during validation so results are more consistent and comparable.
            inv_y_pred = force(model, valid_kwargs=model_args, **y)
        # end torch.no_grad()
...
def train(args, rank, group_name, hparams):
    ...
    print("Initializing VDVAETTS Loss func.")
    criterion = VDVAETTSLoss(hparams)
    
    print("Initializing VDVAETTS Logger.")
