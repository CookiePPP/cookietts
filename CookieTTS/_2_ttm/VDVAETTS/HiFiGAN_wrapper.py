...
class HiFiGAN(nn.Module):
    def __init__(self, hparams=None, input_dim=None):
        super(HiFiGAN, self).__init__()
        if hparams is None:
            return
        assert hparams.batch_size >= hparams.HiFiGAN_batch_size, 'HiFiGAN batch size must be even or greater than VDVAETTS batch size!'
        self.input_dim = input_dim or hparams.dec_bottleneck_dim
        ...
        self.n_models_to_keep = 4# sync with tacotron n_models_to_keep if it's added
    ...
    def load_state_dict_from_file(self, path:str, device='cuda'):
        ...
        if os.path.isdir(path):
            ...
            self.generator        = Generator(h, input_dim=self.input_dim).to(device)
    ...
    def generator_loss(self, gt_audio, pred_audio, gt_mel, pred_mel, loss_dict):
        ...
        y_df_hat_r, y_df_hat_g, fmap_f_r, fmap_f_g = self.mp_discriminator(gt_audio.requires_grad_(), pred_audio,)
        y_ds_hat_r, y_ds_hat_g, fmap_s_r, fmap_s_g = self.ms_discriminator(gt_audio.requires_grad_(), pred_audio,)
    ...
    def forward(self, pred:dict, gt:dict, reduced_loss_dict:dict, loss_dict:dict, loss_scalars:dict):
        ...
        if pred['hifigan_indexes'].sum():
            ...
            # discriminator
            loss, loss_disc_s, loss_disc_f = self.discriminator_loss(gt_audio.requires_grad_(), pred_audio.detach().requires_grad_())
