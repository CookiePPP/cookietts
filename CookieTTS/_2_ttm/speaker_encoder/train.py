...
from loss_function import AutoVCLoss
...
from logger import AutoVCLogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = AutoVCLogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def train(args, rank, group_name, hparams):
    ...
    print("Initializing AutoVC Loss func.")
    criterion = AutoVCLoss(hparams)
    
    print("Initializing AutoVC Logger.")
    ...
    if args.checkpoint_path is not None:
        if args.warm_start_force:
            model, iteration, saved_lookup = warm_start_force_model(args.checkpoint_path, model)
        else:
    ...
    dataloader_args = [*get_args(criterion.forward), *model_args, 'parallel_speaker_embed','non_parallel_speaker_embed']
    if rank == 0:
         ...
         "postnet_MFSE_weight": postnet_MFSE_weight,
                    ...
                    if rank == 0:
                        ...
                        if not is_overflow:
                            ...
                            code_l1      = expavg_loss_dict.get('code_L1' , None) or reduced_loss_dict.get('code_L1' , 0.0)
                            tqdm.write(
                    ...
                    if (iteration % int(validation_interval) == 0) or (os.path.exists(save_file_check_path)) or (iteration < 1000 and (iteration % 250 == 0)):
                        ...
                        val_loss, best_val_loss_dict, file_losses = validate(hparams, args, file_losses, loss_scalars, model, criterion, valset, best_val_loss_dict, iteration, collate_fn, logger,)# validate/teacher_force
                        file_losses = write_dict_to_file(file_losses, os.path.join(args.output_directory, 'file_losses.csv'), args.n_gpus, rank)
                        if use_scheduler:
