...
from model import FESV_AE, load_model
...
from loss_function import FESV_AELoss
from logger import FESV_AELogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = FESV_AELogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def validate(hparams, args, file_losses, loss_scalars, model, criterion, valset, best_val_loss_dict, iteration,
    ...
    with torch.no_grad():
        ...
        for i, batch in tqdm(enumerate(val_loader), desc="Validation", total=len(val_loader), smoothing=0): # i = index, batch = stuff in array[i]
            ...
            loss_dict, file_losses_batch = criterion(model, y_pred, y, loss_scalars, calc_alignments=True, save_alignments=save_alignments)
...
def train(args, rank, group_name, hparams):
    ...
    print("Initializing FESV_AE Loss func.")
    criterion = FESV_AELoss(hparams)
    
    print("Initializing FESV_AE Logger.")
        ...
        try:
            for epoch in tqdm(range(epoch_offset, hparams.epochs), initial=epoch_offset, total=hparams.epochs, desc="Epoch:", position=1, unit="epoch"):
                ...
                for i, batch in tqdm(enumerate(train_loader), desc="Iter:  ", smoothing=0, total=len(train_loader), position=0, unit="iter"):
                    ...
                    loss_dict, file_losses_batch = criterion(model, y_pred, y, loss_scalars, save_alignments=save_alignments)
