...
# https://github.com/gothiswaysir/Transformer_Multi_encoder/blob/952868b01d5e077657a036ced04933ce53dcbf4c/nets/pytorch_backend/e2e_tts_RVAETTS.py#L28-L156
...
class RVAETTSLoss(nn.Module):
    def __init__(self, hparams):
        super(RVAETTSLoss, self).__init__()
    ...
    def forward(self, iteration, model, pred, gt, loss_scalars, hifiGAN=None,):
        ...
        if 'varpred_pred_fesvd' in pred:
            ...
            pred_fesvd = pred['varpred_pred_fesvd'].transpose(1, 2)# [B, 8, txt_T]
            trgt_fesvd = pred['bn_fesvd']                          # [B, 8, txt_T]
            pred_fesvd.masked_fill(~text_mask, 0.0)
            trgt_fesvd.masked_fill(~text_mask, 0.0)
            scalar = text_mask.numel()/text_mask.sum()
            loss_dict['fesvd_MAE'] = F. l1_loss(pred_fesvd, trgt_fesvd)*scalar
            loss_dict['fesvd_MSE'] = F.mse_loss(pred_fesvd, trgt_fesvd)*scalar
        
        if 'varpred_z_mulogvar' in pred:# KL Divergence
            mu, logvar = pred['varpred_z_mulogvar'].chunk(2, dim=1)# [B, n_tokens], [B, n_tokens]
            loss_dict['varpred_KLD'] = -0.5 * torch.mean(1 +logvar -mu.pow(2) -logvar.exp())
            del mu, logvar
        
        if 'vmelenc_logvar' in pred:# KL Divergence
            mu, logvar = pred['vmelenc_logvar'].chunk(2, dim=1)# -> [B, n_tokens, txt_T], [B, n_tokens, txt_T]
            loss_dict['vmelenc_KLD'] = -0.5 * torch.mean(1 +logvar -mu.pow(2) -logvar.exp())
            del mu, logvar
