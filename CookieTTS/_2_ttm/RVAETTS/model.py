...
from CookieTTS.utils.model.layers import ConvNorm, ConvNorm2D, LinearNorm, LSTMCellWithZoneout, SegLSTMWithZoneout
...
from CookieTTS._2_ttm.RVAETTS.arflow import ARFlow
from CookieTTS._2_ttm.RVAETTS.HiFiGAN_wrapper import HiFiGAN
from CookieTTS.utils.model.transformer import PositionalEncoding

drop_rate = 0.5

def load_model(hparams, device='cuda'):
    model = RVAETTS(hparams)
...
class VariationalMelEncoder(nn.Module):# Mix everything from the text half of the model into a single tensor. (This is normally the last step before the text is used for decoding)
    """
    Encodes mel spectrogram frames into latents. Seperate set of latents for each encoder token.
    """
    def __init__(self, hparams, mem_dim):
        super(VariationalMelEncoder, self).__init__()
        self.n_tokens = hparams.vmelenc_n_tokens
        self.std = 0.95
        
        self.seglstm_dim      = hparams.vmelenc_seglstm_dim
        self.melenc = ConvNorm(hparams.n_mel_channels, self.seglstm_dim, 1)
        self.seglstm_n_layers = hparams.vmelenc_seglstm_n_layers
        self.seglstm = SegLSTMWithZoneout(self.seglstm_dim, self.seglstm_dim, self.seglstm_n_layers)
        
        self.resblock_n_layers = hparams.vmelenc_resblock_n_layers
        self.resblock_n_dim    = hparams.vmelenc_resblock_n_dim
        self.resblock_n_blocks = hparams.vmelenc_resblock_n_blocks
        self.resblock_kernel_w = hparams.vmelenc_resblock_kernel_w
        self.resblock = ResBlock1d(self.seglstm_dim+mem_dim+8, 2*self.n_tokens,
                                   self.resblock_n_layers, self.resblock_n_dim,
                                   self.resblock_n_blocks, self.resblock_kernel_w, dropout=0.25,)
        
        self.lin_proj = ConvNorm(self.n_tokens, mem_dim, 1)
    ...
    def forward(self, memory,# [B, txt_T, mem_dim]
               speaker_embed,# [B, embed]
                       fesvd,# [B, 8, txt_T]
                      gt_mel,# [B, n_mel, mel_T]
                 gt_char_dur,):# [B, txt_T]
        melenc_out = self.melenc(gt_mel)# [B, n_mel, mel_T] -> [B, lstm_dim, mel_T]
        seglstm_out = self.seglstm(melenc_out.permute(2, 0, 1), gt_char_dur.round().transpose(0, 1))# [B, lstm_dim, mel_T], [B, txt_T] -> 
        seglstm_out = seglstm_out[2].transpose(1, 2)# -> [B, lstm_dim, txt_T]
        
        resblock_input = torch.cat((seglstm_out, memory.transpose(1, 2), fesvd), dim=1)
        z_mulogvar = self.resblock(resblock_input) # -> [B, 2*n_tokens, txt_T]
        z_mu, z_logvar = z_mulogvar.chunk(2, dim=1)# -> [B,   n_tokens, txt_T], [B, n_tokens, txt_T]
        
        z = self.reparameterize(z_mu, z_logvar)# -> [B, n_tokens, txt_T]
        z_embed = self.lin_proj(z)# [B, n_tokens, txt_T]
        return z_embed, z_mulogvar# [B, n_tokens, txt_T], [B, 2*n_tokens, txt_T]
    
    def infer(self, memory):# [B, txt_T, mem_dim]
        B, txt_T, mem_dim = memory.shape
        return self.lin_proj(memory.new_empty(B, self.n_tokens, txt_T).normal_()*self.std)
...
class VariancePredictor(nn.Module):
    ...
    def __init__(self, hparams, mem_dim):
        ...
        self.std = 0.95
        self.n_tokens = hparams.varpred_n_tokens
        
        self.lstm_dim      = hparams.varpred_lstm_dim
        self.lstm_n_layers = hparams.varpred_lstm_n_layers
        self.enc_LSTM = nn.LSTM(mem_dim+8, self.lstm_dim, self.lstm_n_layers, batch_first=True, dropout=0.1, bidirectional=True)
        self.enc_post = LinearNorm(self.lstm_n_layers*2*self.lstm_dim, 2*self.n_tokens)
        self.enc_proj = LinearNorm(self.n_tokens, mem_dim, 1)
        
        self.dec_LSTM = nn.LSTM(2*mem_dim, self.lstm_dim, self.lstm_n_layers, batch_first=True, dropout=0.1, bidirectional=True)
        self.dec_proj = LinearNorm(2*self.lstm_dim, 8, 1)
    ...
    def forward(self, memory, fesvd):# [B, txt_T, mem_dim], [B, 8, txt_T]
        B, txt_T, mem_dim = memory.shape
        enc_input = torch.cat((memory, fesvd.transpose(1, 2)), dim=2)# [B, txt_T, mem_dim+8]
        _, (enc_out, _) = self.enc_LSTM(enc_input)
        enc_out = enc_out.view(self.lstm_n_layers, 2, B, self.lstm_dim)# [n_layers, 2, B, lstm_dim]
        enc_out = enc_out.permute(2, 0, 1, 3).reshape(B, -1)# [B, 2*n_layers*lstm_dim]
        z_mulogvar = self.enc_post(enc_out)# [B, 2*n_tokens]
        z_mu, z_logvar = z_mulogvar.chunk(2, dim=1)# [B, n_tokens], [B, n_tokens]
        z = self.reparameterize(z_mu, z_logvar)# [B, n_tokens]
        embed = self.enc_proj(z)# [B, embed]
        
        dec_input = torch.cat(torch.broadcast_tensors(memory, embed.unsqueeze(1)), dim=2)# [B, txt_T, mem_dim], [B, 1, embed] -> [B, txt_T, mem+embed]
        dec_out   = self.dec_LSTM(dec_input)[0]
        pred_fesvd = self.dec_proj(dec_out)# [B, txt_T, 8]
        return pred_fesvd, z_mulogvar# [B, txt_T, 8], [B, 2*n_tokens]
    
    def infer(self, memory, std=None):# [B, txt_T, mem_dim]
        if std is not None:
            self.std = std
        
        B, txt_T, mem_dim = memory.shape
        embed = self.enc_proj(memory.new_empty(B, self.n_tokens).normal_()*self.std)
        
        dec_input = torch.cat(torch.broadcast_tensors(memory, embed.unsqueeze(1)), dim=1)# [B, txt_T, mem_dim], [B, 1, embed] -> [B, txt_T, mem+embed]
        dec_out   = self.dec_LSTM(dec_input)[0]
        pred_fesvd = self.dec_proj(dec_out)# [B, txt_T, lstm_dim]
        return pred_fesvd
...
class RVAETTS(nn.Module):# Main module, contains all the submodules for the network.
    def __init__(self, hparams):
        super(RVAETTS, self).__init__()
        ...
        if True:# Variational Char-Level Mel Encoder
            self.vmelenc = VariationalMelEncoder(hparams, mem_dim)
        ...
        if True:# Conditional Scale Shift the memory with FESV+logdur
            self.cond_ss = SSSUnit(mem_dim, self.fesvd_dim+mem_dim)
    ...
    def forward(self, gt_audio, audio_lengths,# FloatTensor[B, wav_T],  LongTensor[B]
        ...
        if hasattr(self, 'varpred'):
            pred_fesvd, z_mulogvar = self.maybe_cp(self.varpred, *(memory, bn_fesvd))# [B, txt_T, 8], [B, 2*n_tokens]
            outputs['varpred_pred_fesvd'] = pred_fesvd
            outputs['varpred_z_mulogvar'] = z_mulogvar
            del z_mulogvar
        
        if hasattr(self, 'vmelenc'):# Variational Mel Encoder (with char-level tokens)
            z_embed, z_mulogvar = self.maybe_cp(self.vmelenc, *(memory, speaker_embed, bn_fesvd, gt_mel, gt_char_dur))# -> [B, n_tokens, txt_T], [B, 2*n_tokens, txt_T]
            outputs['vmelenc_logvar'] = z_mulogvar
            del z_mulogvar
        
        # Conditional Scale Shift
        cond = torch.cat((bn_fesvd.transpose(1, 2), z_embed.transpose(1, 2)), dim=-1)# ([B, txt_T, 8], []) -> [B, txt_T, 
        memory = self.cond_ss(memory, cond)# [B, txt_T, mem_dim]
    ...
    def inference(self, text_seq, text_lengths, speaker_id, torchmoji_hdn,
        ...
        if bn_fesvd is None:
            z = torch.empty(memory.shape[0], self.fesvd_dim, memory.shape[1], device=memory.device, dtype=memory.dtype).normal_()*char_z_sigma
            z_padded = F.pad(z, (trim_chars, trim_chars))# [B, txt_T, fesvd_dim] -> [B, txt_T+2*pad, fesvd_dim]
            ...
            outputs['pred_bn_fesvd'] = bn_fesvd = pred_bn_fesvd[:, :, trim_chars:-trim_chars]# [B, fesvd_dim, txt_T]
