...
from model import RVAETTS, load_model
...
from loss_function import RVAETTSLoss
from logger import RVAETTSLogger
...
def prepare_directories_and_logger(hparams, args):
    if args.rank == 0:
        ...
        logger = RVAETTSLogger(os.path.join(args.output_directory, args.log_directory), hparams)
    else:
        logger = None
    return logger
...
def train(args, rank, group_name, hparams):
    ...
    print("Initializing RVAETTS Loss func.")
    criterion = RVAETTSLoss(hparams)
    
    print("Initializing RVAETTS Logger.")
            ...
            for epoch in tqdm(range(epoch_offset, hparams.epochs), initial=epoch_offset, total=hparams.epochs, desc="Epoch:", position=1, unit="epoch"):
                ...
                for i, batch in tqdm(enumerate(train_loader), desc="Iter:  ", smoothing=0, total=len(train_loader), position=0, unit="iter"):
                    ...
                    loss_scalars = {
                      ...
                      "vmelenc_KLD_weight": vmelenc_KLD_weight,
