...
class Generator(nn.Module):
    def __init__(self, h):
        ...
        #enable_ddpm
        #enable_bclassifier
        #enable_sss_spkr_cond
        
        self.enable_hdn_cascade = h.enable_hdn_cascade
        self.enable_cascade = h.enable_cascade
        if h.enable_cascade:
            self.durgenerator = FFT(h.dur_hidden_dim, h.dur_n_heads, h.dur_ff_dim, h.dur_n_layers, ff_kernel_size=h.dur_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim+0, output_dim=1+(h.mem_dim*self.enable_hdn_cascade))
            self.nrggenerator = FFT(h.nrg_hidden_dim, h.nrg_n_heads, h.nrg_ff_dim, h.nrg_n_layers, ff_kernel_size=h.nrg_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim+1, output_dim=1+(h.mem_dim*self.enable_hdn_cascade))
            self.cf0generator = FFT(h.cf0_hidden_dim, h.cf0_n_heads, h.cf0_ff_dim, h.cf0_n_layers, ff_kernel_size=h.cf0_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim+2, output_dim=1)
        else:
            self.aiogenerator = FFT(h.aio_hidden_dim, h.aio_n_heads, h.aio_ff_dim, h.aio_n_layers, ff_kernel_size=h.aio_ff_kernel_size, learned_noise=True, add_position_encoding=True, input_dim=h.mem_dim, output_dim=3)
    
    def forward(self, gt_char_logf0,# FloatTensor[B, txt_T]
                     gt_char_voiced,# FloatTensor[B, txt_T]
                     gt_char_energy,# FloatTensor[B, txt_T]
                 text, text_lengths,#  LongTensor[B, txt_T],        LongTensor[B]
                         speaker_id,#  LongTensor[B]
                 speaker_f0_meanstd,# FloatTensor[B, 2]
              speaker_slyps_meanstd,# FloatTensor[B, 2]
        ...
        if self.enable_cascade:
            if self.enable_hdn_cascade:
                pr_logdur, encoder_outputs = self.durgenerator(encoder_outputs, text_lengths).split((1, self.mem_dim), dim=-1)# [B, txt_T, mem_dim] -> [B, txt_T, 1], [B, txt_T, mem_dim]
                pr_energy, encoder_outputs = self.nrggenerator(
                    torch.cat((encoder_outputs, pr_logdur), dim=-1), text_lengths).split((1, self.mem_dim), dim=-1)# [B, txt_T, mem_dim] -> [B, txt_T, 1], [B, txt_T, mem_dim]
                pr_logcf0                  = self.cf0generator(
                    torch.cat((encoder_outputs, pr_logdur, pr_energy), dim=-1), text_lengths)# [B, txt_T, mem_dim] -> [B, txt_T, 1]
            else:
                pr_logdur = self.durgenerator(encoder_outputs, text_lengths)# [B, txt_T, mem_dim] -> [B, txt_T, 1]
                pr_energy = self.nrggenerator(
                    torch.cat((encoder_outputs, pr_logdur), dim=-1), text_lengths)# [B, txt_T, mem_dim] -> [B, txt_T, 1]
                pr_logcf0 = self.cf0generator(
                    torch.cat((encoder_outputs, pr_logdur, pr_energy), dim=-1), text_lengths)# [B, txt_T, mem_dim] -> [B, txt_T, 1]
        else:
            pr_outputs = self.aiogenerator(encoder_outputs, text_lengths)# [B, txt_T, mem_dim] -> [B, txt_T, 3]
            pr_logdur, pr_energy, pr_logcf0 = pr_outputs.chunk(3, dim=-1)
        out['pr_logdur'] = pr_logdur
        out['pr_energy'] = pr_energy
    ...
    def infer(self, text, text_lengths, torchmoji_hdn, speaker_id, speaker_f0_meanstd, speaker_slyps_meanstd, hard_alignments=None, sigma=1.0, randn_max=99.9, max_decode_length=1024):
        ...
        # DurF0s Generator
        pr_durlogf0voiced = self.durgenerator(encoder_outputs, text_lengths)# [B, txt_T, mem_dim] -> [B, 3, txt_T]
        pr_dur, pr_char_logf0, pr_char_voiced = pr_durlogf0voiced.transpose(1, 2).chunk(3, dim=1)# -> [B, 1, txt_T], [B, 1, txt_T], [B, 1, txt_T]
        
        pr_char_voiced = pr_char_voiced.sigmoid()*get_mask_from_lengths(text_lengths).unsqueeze(1)
        
        pr_dur = F.relu(pr_dur.squeeze(1)).round()# -> [B, txt_T]
        mel_lengths = pr_dur.sum(1).long()# -> LongTensor[B]
        out['mel_lengths'] = mel_lengths
        
        pr_durlogf0voiced = torch.cat((pr_dur.unsqueeze(1), pr_char_logf0*(pr_char_voiced>0.01), pr_char_voiced), dim=1)# [B, 3, txt_T]
        
        # Attention
        encoder_outputs = encoder_outputs.transpose(1, 2)# [B, txt_T, mem_dim] -> [B, mem_dim, txt_T]
        if hard_alignments is not None:
            attention_contexts = encoder_outputs @ hard_alignments.transpose(1, 2)# [B, mem_dim, txt_T] @ [B, txt_T, mel_T] -> [B, mem_dim, mel_T]
        else:
            attention_contexts = self.get_attention_from_lengths(encoder_outputs.transpose(1, 2), pr_dur, text_lengths).transpose(1, 2)# -> [B, mem_dim, mel_T]
        
        # F0s Generator
        if hard_alignments is not None:
            pr_durlogf0voiced = pr_durlogf0voiced @ hard_alignments.transpose(1, 2)# [B, 3, txt_T] @ [B, txt_T, mel_T] -> [B, 3, mel_T]
        else:
            pr_durlogf0voiced = self.get_attention_from_lengths(pr_durlogf0voiced.transpose(1, 2), pr_dur, text_lengths).transpose(1, 2)# -> [B, 3, mel_T]
        
        f0sgen_input = torch.cat((attention_contexts, pr_durlogf0voiced), dim=1)# -> [B, mem_dim+nrg_dim, mel_T]
        pr_logf0svoiced = self.f0sgenerator(f0sgen_input.transpose(1, 2), mel_lengths)# [B, mem_dim+nrg_dim, mel_T] -> [B, mel_T, 2*nrg_dim]
        pr_logf0s, pr_energy = pr_logf0svoiced.transpose(1, 2).chunk(2, dim=1)# -> [B, nrg_dim, mel_T], [B, nrg_dim, mel_T]
        pr_energy = pr_energy.sigmoid()*get_mask_from_lengths(mel_lengths).unsqueeze(1)
        pr_logf0s = pr_logf0s*pr_energy.round()# [B, nrg_dim, mel_T]
        
        # Mel Generator
        melgen_input = torch.cat((attention_contexts, pr_durlogf0voiced, pr_logf0s), dim=1)# -> [B, mem_dim+nrg_dim, mel_T]
        pr_mel = self.melgenerator(melgen_input.transpose(1, 2), mel_lengths).transpose(1, 2)# [B, mem_dim+nrg_dim, mel_T] -> [B, n_mel, mel_T]
...
class Discriminator(nn.Module):
    def __init__(self, h):
        ...
        self.durdiscriminator = FFT(h.d_dur_hidden_dim, h.d_dur_n_heads, h.d_dur_ff_dim, h.d_dur_n_layers, ff_kernel_size=h.d_dur_ff_kernel_size, n_pre_layers=getattr(h, 'd_dur_n_pre_layers', 0), add_position_encoding=True, input_dim=h.mem_dim+3,            output_dim=1)
        self.f0sdiscriminator = FFT(h.d_nrg_hidden_dim, h.d_nrg_n_heads, h.d_nrg_ff_dim, h.d_nrg_n_layers, ff_kernel_size=h.d_nrg_ff_kernel_size, n_pre_layers=getattr(h, 'd_nrg_n_pre_layers', 0), add_position_encoding=True, input_dim=h.mem_dim+self.n_f0s+3, output_dim=1)
        ...
        self.meldiscriminator = FFT(h.d_mel_hidden_dim, h.d_mel_n_heads, h.d_mel_ff_dim, h.d_mel_n_layers, ff_kernel_size=h.d_mel_ff_kernel_size, n_pre_layers=getattr(h, 'd_mel_n_pre_layers', 0), add_position_encoding=True, input_dim=h.mem_dim+self.n_f0s+h.d_mel_hidden_dim, output_dim=1)
    
    def discriminator_loss(self, gt, pr_encoder_outputs, pr_durlogf0voiced, pr_f0s, pr_mel, hard_alignments):
        ((pr_durlogf0voiced_realness, gt_durlogf0voiced_realness),
         (           pr_nrg_realness,            gt_nrg_realness),
         (           pr_mel_realness,            gt_mel_realness)) = self(gt, pr_encoder_outputs, pr_durlogf0voiced, pr_f0s, pr_mel, hard_alignments)
        
        dur_loss = discriminator_loss(pr_durlogf0voiced_realness, gt_durlogf0voiced_realness, gt['text_lengths'])
        nrg_loss = discriminator_loss(           pr_nrg_realness,            gt_nrg_realness,  gt['mel_lengths'])
        mel_loss = discriminator_loss(           pr_mel_realness,            gt_mel_realness,  gt['mel_lengths'])
        
        return dur_loss, nrg_loss, mel_loss, (
         (pr_durlogf0voiced_realness, gt_durlogf0voiced_realness),
         (           pr_nrg_realness,            gt_nrg_realness),
         (           pr_mel_realness,            gt_mel_realness)
        )
    
    def forward(self, gt, pr_encoder_outputs, pr_durlogf0voiced, pr_f0s, pr_mel, hard_alignments):
        gt_char_dur = hard_alignments.detach().sum(dim=1)# [B, mel_T, txt_T] -> [B, txt_T]
        gt_char_dur += gt_char_dur.new_empty(gt_char_dur.shape).uniform_(-0.5, 0.5)
        gt_f0s = gt['gt_lognrg_gated']
        gt_mel = gt['gt_mel']
        mel_noise = gt_mel.new_empty(gt_mel.shape).normal_(std=0.3)
        
        gt_durlogf0voiced = torch.stack((
                gt_char_dur,
                gt['gt_char_logf0' ]+gt['gt_char_logf0' ].new_empty(gt['gt_char_logf0' ].shape).normal_(std=2.5),
                gt['gt_char_voiced']+gt['gt_char_voiced'].new_empty(gt['gt_char_voiced'].shape).normal_(std=2.5)), dim=1)# [B, txt_T], [B, txt_T], [B, txt_T] -> [B, 3, txt_T]
        pr_durlogf0voiced = torch.stack((
                pr_durlogf0voiced[:, 0, :]+pr_durlogf0voiced[:, 0, :].new_empty(pr_durlogf0voiced[:, 0, :].shape).uniform_(-0.5, 0.5),
                pr_durlogf0voiced[:, 1, :]+pr_durlogf0voiced[:, 1, :].new_empty(pr_durlogf0voiced[:, 1, :].shape).normal_(std=2.5),
                pr_durlogf0voiced[:, 2, :]+pr_durlogf0voiced[:, 2, :].new_empty(pr_durlogf0voiced[:, 2, :].shape).normal_(std=2.5)), dim=1)# [B, txt_T], [B, txt_T], [B, txt_T] -> [B, 3, txt_T]
        # mask dur tensor
        #text_mask = ~get_mask_from_lengths(gt['text_lengths']).unsqueeze(1)# [B, 1, txt_T]
        #gt_c_dmask = (  gt_durlogf0voiced[:, 0:1] < 0.50) & (  gt_durlogf0voiced[:, 0:1] >= 0.0)
        #gt_c_vmask = (  gt_durlogf0voiced[:, 2:3] < 0.01) & (  gt_durlogf0voiced[:, 2:3] >= 0.0)
        #pr_c_dmask = (pr_durlogf0voiced[:, 0:1] < 0.50) & (pr_durlogf0voiced[:, 0:1] >= 0.0)
        #pr_c_vmask = (pr_durlogf0voiced[:, 2:3] < 0.01) & (pr_durlogf0voiced[:, 2:3] >= 0.0)
        #padded_mask = text_mask | gt_c_dmask | gt_c_vmask | pr_c_dmask | pr_c_vmask
        #gt_durlogf0voiced   =   gt_durlogf0voiced.masked_fill(padded_mask, 0.0)# [B, 3, txt_T]
        #pr_durlogf0voiced = pr_durlogf0voiced.masked_fill(padded_mask, 0.0)# [B, 3, txt_T]
        ...
        pr_durlogf0voiced_realness = self.durdiscriminator(torch.cat((pr_durlogf0voiced, encoder_outputs.transpose(1, 2)), dim=1).transpose(1, 2), gt['text_lengths'])
        gt_durlogf0voiced_realness   = self.durdiscriminator(torch.cat((  gt_durlogf0voiced, encoder_outputs.transpose(1, 2)), dim=1).transpose(1, 2), gt['text_lengths'])
        ...
        attention_contexts      =   encoder_outputs @ hard_alignments.transpose(1, 2)# [B, mem_dim, txt_T] @ [B, txt_T, mel_T] -> [B, mem_dim, mel_T]
        gt_frame_durlogf0voiced = gt_durlogf0voiced @ hard_alignments.transpose(1, 2)# [B,       3, txt_T] @ [B, txt_T, mel_T] -> [B,       3, mel_T]
        
        pr_nrg_realness = self.f0sdiscriminator(torch.cat((pr_f0s, gt_frame_durlogf0voiced, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        gt_nrg_realness   = self.f0sdiscriminator(torch.cat((  gt_f0s, gt_frame_durlogf0voiced, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        ...
        pr_mel_realness = self.meldiscriminator(torch.cat((pr_mel_decoded, gt_f0s, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        gt_mel_realness   = self.meldiscriminator(torch.cat((  gt_mel_decoded, gt_f0s, attention_contexts), dim=1).transpose(1, 2), gt['mel_lengths'])
        
        if self.out_tanh:
            pr_durlogf0voiced_realness, gt_durlogf0voiced_realness = pr_durlogf0voiced_realness.tanh()*0.5, gt_durlogf0voiced_realness.tanh()*0.5
            pr_nrg_realness, gt_nrg_realness = pr_nrg_realness.tanh()*0.5, gt_nrg_realness.tanh()*0.5
            pr_mel_realness, gt_mel_realness = pr_mel_realness.tanh()*0.5, gt_mel_realness.tanh()*0.5
        
        return ((pr_durlogf0voiced_realness, gt_durlogf0voiced_realness),
                (           pr_nrg_realness,            gt_nrg_realness),
                (           pr_mel_realness,            gt_mel_realness))
...
class Model(nn.Module):
    def __init__(self, h):
        ...
        self.enable_vae_encoder   = h.enable_vae_encoder   # enable VAE encoder
        self.enable_discriminator = h.enable_discriminator # enable GAN discriminator
        self.enable_ddpm          = h.enable_ddpm          # enable Denoising Diffusion Probabilistic Model objective
        self.enable_bclassifier   = h.enable_bclassifier   # enable Bucketed Classifier outputs
        self.enable_cascade       = h.enable_cascade       # enable Cascade - split the network into sections that feed into each other for each output type
                                                           #                - e.g: Predict duration, use that to predict voiced, use those 2 to predict f0.
        self.enable_hdn_cascade   = h.enable_hdn_cascade   # enable hidden state Cascading - If Cascade enabled, feed hidden states from previous sections into next sections as well.
        
        if h.enable_vae_encoder:
            self.vaeencoder = VAEEncoder(h)
        
        self.generator = Generator(h)
        
        if h.enable_discriminator:
    ...
    def generator_loss(self, gt, pr_encoder_outputs, pr_durf0s, pr_f0s, pr_mel, hard_alignments):
        ((pr_ainone_realness, gt_ainone_realness),
         (pr_logdur_realness, gt_logdur_realness),
         (pr_lognrg_realness, gt_lognrg_realness),
         (pr_energy_realness, gt_voiced_realness)) = self.discriminator(gt, pr_encoder_outputs, pr_durf0s, pr_f0s, pr_mel, hard_alignments)
        if self.enable_cascade:
            aio_loss = generator_loss(pr_ainone_realness, gt['text_lengths'])
        else:
            logdur_loss = generator_loss(pr_logdur_realness, gt['text_lengths'])
            lognrg_loss = generator_loss(pr_lognrg_realness, gt['text_lengths'])
            voiced_loss = generator_loss(pr_energy_realness, gt['text_lengths'])
        
        return aio_loss, logdur_loss, lognrg_loss, voiced_loss, (
         (pr_logdur_realness, gt_logdur_realness),
         (pr_lognrg_realness, gt_lognrg_realness),
         (pr_energy_realness, gt_voiced_realness),
        )
    ...
    def forward(self,
        ...
        out_tmp = self.generator(gt_char_logf0, gt_char_voiced, gt_char_energy, text, text_lengths, speaker_id, speaker_f0_meanstd, speaker_slyps_meanstd, gt_sylps, torchmoji_hdn, freq_grad_scalar, alignment)
        out = {**out, **out_tmp}
