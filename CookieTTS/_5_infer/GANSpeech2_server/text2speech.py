...
from CookieTTS._2_ttm.GANSpeech.model import load_model as load_model_gs
from CookieTTS._2_ttm.GANSpeech2.model import load_model as load_model_gs2
...
def parse_text_into_segments(texts, target_segment_len=120, split_at_quotes=True, split_at_newline=True, split_at_commas=True):
    texts = (texts.strip()
    ...
    while len(rev_texts):
        ...
        if len(text) <= target_segment_len:
            ...
            if any(x in text for x in EOS_punc):
                ...
                if len(tmp) <= target_segment_len:
                    ...
                    continue
            if ',' in text and split_at_commas:
...
class T2S:
    def __init__(self, conf):
        ...
        assert self.ttm_current in self.conf['TTM']['models'].keys(), "GANSpeech2 default model not found in config models"
        GANSpeech2_path = self.conf['TTM']['models'][self.ttm_current]['modelpath'] # get first available GANSpeech2
        self.GANSpeech2, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup, self.ttm_sp_f0_meanstd, self.ttm_sp_sylps_meanstd = self.load_GANSpeech2(GANSpeech2_path)
    ...
    def load_hifigan(self, vocoder_path):
        ...
        from CookieTTS._4_mtw.hifigan_ct.model import load_generator_from_path as load_hifigan_model
    ...
    def load_GANSpeech2(self, GANSpeech2_path):
        """Loads GANSpeech2,
        Returns:
        - model
        - hparams
        - speaker_lookup
        """
        checkpoint = torch.load(GANSpeech2_path) # load file into memory
        print("Loading GANSpeech2... ", end="")
        checkpoint_hparams = checkpoint.get('h', None) or checkpoint['hparams'] # get hparams
        checkpoint_dict    = checkpoint['state_dict'] # get state_dict
        
        if hasattr(checkpoint_hparams, 'cf0_hidden_dim'):
            model = load_model_gs(checkpoint_hparams) # initialize the model
        elif hasattr(checkpoint_hparams, 'f0s_hidden_dim'):
            model = load_model_gs2(checkpoint_hparams)# initialize the model
        model.load_state_dict(checkpoint_dict) # load pretrained weights
        _ = model.cuda().eval()#.half()
        print("Done")
        
        speaker_name_lookup  = checkpoint['speaker_name_lookup']  # save speaker name lookup
        speaker_id_lookup    = checkpoint['speaker_id_lookup']    # save speaker_id lookup
        speaker_f0_meanstd   = checkpoint['speaker_f0_meanstd']   # save speaker_id lookup
        speaker_sylps_meanstd= checkpoint['speaker_sylps_meanstd']# save speaker_id lookup
        
        print(f"This GANSpeech2 model has been trained for {checkpoint['iteration']} Iterations.")
        return model, checkpoint_hparams, speaker_name_lookup, speaker_id_lookup, speaker_f0_meanstd, speaker_sylps_meanstd
    
    
    def update_gs(self, GANSpeech2_name):
        self.GANSpeech2, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup, self.ttm_sp_f0_meanstd, self.ttm_sp_sylps_meanstd = self.load_GANSpeech2(self.conf['TTM']['models'][GANSpeech2_name]['modelpath'])
        self.ttm_current = GANSpeech2_name
        
        if self.conf['TTM']['models'][GANSpeech2_name]['use_speaker_ids_file_override']:# (optional) override
            self.ttm_sp_name_lookup = {name.strip(): self.ttm_sp_id_lookup[int(ext_id.strip())] for _, name, ext_id, *_ in load_filepaths_and_text(self.conf['TTM']['models'][GANSpeech2_name]['speaker_ids_file'])}
    ...
    def infer(self, text, speaker, use_arpabet, cat_silence_s,
              ...
              target_score=0.75, absolutely_required_score=-float('inf'), absolute_maximum_tries=1):
        """
        PARAMS:
        ...
        ...
        if hasattr(self, 'calls'):
            ...
            self.calls = 1
        print(f"\n\nCURRENT CALLS: {self.calls}")
        
        #  tmp
        rev_ttm_sp_id_lookup = {v: k for k,v in self.ttm_sp_id_lookup.items()}
        ...
        max_focus_weighting   = 0.5 # 'stuck factor', a penalty for clips that spend execisve time on the same letter.
        min_focus_weighting   = 0.0 # 'miniskip factor', a penalty for skipping/ignoring single letters in the input text.
        ...
        # update GANSpeech2 stopping params
        frames_per_second = float(self.ttm_hparams.sampling_rate/self.ttm_hparams.hop_length)
        max_decode_length = int(min(max([len(t) for t in texts]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))//self.ttm_hparams.n_frames_per_step
        ...
        for text_index, text in enumerate(texts):
            ...
            max_decode_length = int(min(max([len(t) for t in text_batch]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))//self.ttm_hparams.n_frames_per_step
            ...
            # get speaker_ids
            speaker_ids_ext = [self.ttm_sp_name_lookup[speaker] for speaker in batch_speaker_names]
            #speaker_ids_ext = [rev_ttm_sp_id_lookup[id] for id in speaker_ids_ext]
            speaker_ids = torch.LongTensor(speaker_ids_ext).cuda()
            ...
            style_input = style_input.to(next(self.GANSpeech2.parameters()).dtype)
            ...
            # get f0 mean_std and sylps mean_std tensors
            speaker_f0_meanstd    = torch.stack([torch.tensor(self.ttm_sp_f0_meanstd   [speaker_id_ext]) for speaker_id_ext in speaker_ids_ext], dim=0).cuda()# [[2],]*B -> [B, 2]
            speaker_sylps_meanstd = torch.stack([torch.tensor(self.ttm_sp_sylps_meanstd[speaker_id_ext]) for speaker_id_ext in speaker_ids_ext], dim=0).cuda()# [[2],]*B -> [B, 2]
            #speaker_f0_meanstd   [~torch.isfinite(speaker_f0_meanstd   )] = 0.0
            #speaker_sylps_meanstd[~torch.isfinite(speaker_sylps_meanstd)] = 0.0
            print(*speaker_f0_meanstd[0].tolist(), *speaker_sylps_meanstd[0].tolist())
            ...
            # Remove commas, special requirement for GANSpeech2
            text_batch = [text[:-1]+'.' if (text[-1] == ',') else text for text in text_batch]
            ...
            # add any start/stop tokens.
            text_batch = [self.ttm_hparams.start_token+text+self.ttm_hparams.stop_token if (text[-1] not in valid_last_char) else text for text in text_batch]
            
            # convert texts to number representation, pad where appropriate and move to GPU
            sequence_split = [torch.LongTensor(text_to_sequence(text, self.ttm_hparams.text_cleaners)).repeat_interleave(getattr(self.ttm_hparams, 'text_repeat_interleave', 1), dim=0) for text in text_batch] # convert texts to numpy representation
            ...
            sequence     = sequence    .cuda().long()# move to GPU
            text_lengths = text_lengths.cuda().long()# move to GPU
            
            # repeat_interleave if performing multiple inference passes with each text input
            sequence              = sequence             .repeat_interleave(batch_size_per_text, dim=0)
            text_lengths          = text_lengths         .repeat_interleave(batch_size_per_text, dim=0)
            speaker_f0_meanstd    = speaker_f0_meanstd   .repeat_interleave(batch_size_per_text, dim=0)
            speaker_sylps_meanstd = speaker_sylps_meanstd.repeat_interleave(batch_size_per_text, dim=0)
            speaker_ids           = speaker_ids          .repeat_interleave(batch_size_per_text, dim=0)
            
            if status_updates: mf_start=time.time(); print("Running GANSpeech2", end='')
            try:
                best_score = np.ones (simultaneous_texts) * -float('inf')
                ...
                while np.amin(best_score) < target_score:
                    ...
                    outputs = self.GANSpeech2.generator.infer(sequence, text_lengths, style_input, speaker_ids, speaker_f0_meanstd, speaker_sylps_meanstd, sigma=sigma, randn_max=randn_max, max_decode_length=max_decode_length,)
                    pred_mel_batch   = outputs['pred_mel']
                    mel_lengths      = outputs['mel_lengths']# [B]
                    
                    # metric for html side
                    n_passes    += 1
                    total_specs += pred_mel_batch.shape[0]
                    
                    # get alignment metrics for each item
                    #atd = alignment_metric(alignments_batch, input_lengths=text_lengths, output_lengths=(mel_lengths/self.ttm_hparams.n_frames_per_step).ceil().long())
                    
                    # split batch into items
                    batch = list(zip(
                        mel_lengths     .split(1, dim=0),
                        pred_mel_batch  .split(1, dim=0),
                        #pred_gate_batch .split(1, dim=0),
                    ...
                    for j in range(simultaneous_texts): # process each set of text spectrograms seperately
                        ...
                        for k, (mel_length, pred_mel) in enumerate(sametext_batch):
                            ## factors that make up score
                            #weighted_score = avg_prob.item() # general alignment quality
                            #diagonality_punishment = max(max(diagonality.item()-1.20, 0.95-diagonality.item()), 0.0) * 0.5 * diagonality_weighting       # speaking each letter at a similar pace.
                            #max_dur_punishment     = max((enc_max_focus.item()-(60/self.ttm_hparams.n_frames_per_step)), 0) * 0.005 * max_focus_weighting# getting stuck on same letter for 0.5s
                            #min_dur_punishment     = max(0.00-enc_min_focus.item(), 0) * 1.0 * min_focus_weighting                                       # skipping single enc outputs
                            #avg_dur_punishment     = max((3.60/self.ttm_hparams.n_frames_per_step)-enc_avg_focus.item(), 0) * 1.0 * avg_focus_weighting  # skipping most enc outputs
                            #mis_dur_punishment     = max(p_missing_enc.item()-0.07, 0) if text_lengths[j*batch_size_per_text] > 12 else 0.0              # skipping some percent of the text
                            #
                            ...
                            #if torch.isnan(pred_mel).any() or torch.isnan(pred_gate).any() or torch.isnan(alignments).any() or weighted_score == float('nan'):
                            #    print("NaN found", torch.isnan(pred_mel).any(), torch.isnan(pred_gate).any(), torch.isnan(alignments).any(), torch.isnan(alignments).any())
                            ...
                            if weighted_score > best_score[j]:
                                ...
                                best_generations[j] = [pred_mel, mel_length,]
                            else:
                                print("Failed", gtext_batch[j])
                            tries[j]+=1
                    ...
                    if np.amin(tries) < (max_attempts-1):
                        print(f'Minimum score of {np.amin(best_score):.3f} is less than Target score of {target_score}. Retrying.')
                    elif np.amin(best_score) < absolutely_required_score:
                        print(f"Minimum score of {np.amin(best_score):.3f} is less than 'Absolutely Required score' of {absolutely_required_score}. Retrying.")
            except StopIteration:
                del batch
                if status_updates: print(f"\nDone in {time.time()-mf_start:.2f}s")
            
            newline_sym = "\n"
            assert not any([x==0 for x in best_generations]), f'GANSpeech2 Failed to generate the following text(s) after multiple attempts.\n{[x for i, x in enumerate(gtext_batch) if best_generations[i]==0]}\nScore(s): {[x+newline_sym for i, x in enumerate(score_str) if best_generationsp[i]==0]}'
            ...
            # [[mel, gate,], [mel, gate,], [mel, gate,]] -> [[mel, mel, mel], [gate, gate, gate]
            pred_mel_batch = [x[0][0].T for x in best_generations]
            mel_lengths    = torch.stack([x[1][0] for x in best_generations], dim=0)
            # pickup the best attempts from each input
            
            max_mel_length = mel_lengths.max()
            pred_mel_batch = torch.nn.utils.rnn.pad_sequence(pred_mel_batch, batch_first=True, padding_value=0.0 if self.MTW_conf.uses_latent_input else -11.52).transpose(1,2)[:,:,:max_mel_length]
            
            if status_updates:
                vo_start = time.time()
                print("Running Vocoder... ", end='')
            ...
            audio_batch = []
            for i in range(0, len(pred_mel_batch), self.vocoder_batch_size):
                pred_mel_part_batch = pred_mel_batch[i:i+self.vocoder_batch_size]
            ...
            for j, audio in enumerate(audio_batch):
                ...
                audio_end = mel_lengths[j] * self.MTW_conf['hop_size']
                audio     = audio[:, :audio_end]
                
                # remove GANSpeech2 padding
                spec_end = mel_lengths[j]
