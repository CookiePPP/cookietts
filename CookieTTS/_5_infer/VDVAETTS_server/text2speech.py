...
from CookieTTS._2_ttm.VDVAETTS.model import load_model
...
class T2S:
    def __init__(self, conf):
        ...
        assert self.ttm_current in self.conf['TTM']['models'].keys(), "VDVAETTS default model not found in config models"
        VDVAETTS_path = self.conf['TTM']['models'][self.ttm_current]['modelpath'] # get first available VDVAETTS
        self.VDVAETTS, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup = self.load_VDVAETTS(VDVAETTS_path)
    ...
    def load_hifigan(self, vocoder_path):
        ...
        vocoder, vocoder_config = load_hifigan_model(vocoder_path, latent_dim=self.ttm_hparams.dec_bottleneck_dim)
        vocoder.half()
        print("Done!")
    ...
    def load_VDVAETTS(self, VDVAETTS_path):
        """Loads VDVAETTS,
        Returns:
        - model
        - hparams
        - speaker_lookup
        """
        checkpoint = torch.load(VDVAETTS_path) # load file into memory
        print("Loading VDVAETTS... ", end="")
        ...
        VDVAETTS_speaker_name_lookup = checkpoint['speaker_name_lookup'] # save speaker name lookup
        VDVAETTS_speaker_id_lookup   = checkpoint['speaker_id_lookup']   # save speaker_id lookup
        
        print(f"This VDVAETTS model has been trained for {checkpoint['iteration']} Iterations.")
        return model, checkpoint_hparams, VDVAETTS_speaker_name_lookup, VDVAETTS_speaker_id_lookup
    
    
    def update_mf(self, VDVAETTS_name):
        self.VDVAETTS, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup = self.load_VDVAETTS(self.conf['TTM']['models'][VDVAETTS_name]['modelpath'])
        self.ttm_current = VDVAETTS_name
        
        if self.conf['TTM']['models'][VDVAETTS_name]['use_speaker_ids_file_override']:# (optional) override
            self.ttm_sp_name_lookup = {name.strip(): self.ttm_sp_id_lookup[int(ext_id.strip())] for _, name, ext_id, *_ in load_filepaths_and_text(self.conf['TTM']['models'][VDVAETTS_name]['speaker_ids_file'])}
    ...
    def infer(self, text, speaker, use_arpabet, cat_silence_s,
        ...
        # update VDVAETTS stopping params
        frames_per_second = float(self.ttm_hparams.sampling_rate/self.ttm_hparams.hop_length)
        self.VDVAETTS.max_decoder_steps = int(min(max([len(t) for t in texts]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))
        ...
        for text_index, text in enumerate(texts):
            ...
            self.VDVAETTS.max_decoder_steps = int(min(max([len(t) for t in text_batch]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))
            ...
            style_input = style_input.to(next(self.VDVAETTS.parameters()).dtype)
            ...
            if status_updates: mf_start=time.time(); print("Running VDVAETTS", end='')
            try:
            ...
            assert not any([x==0 for x in best_generations]), 'VDVAETTS Failed to generate one of the texts after multiple attempts.'
            ...
            batch_pred_mel = torch.nn.utils.rnn.pad_sequence(batch_pred_mel, batch_first=True, padding_value=0.0 if self.MTW_conf.uses_latent_input else -11.52).transpose(1,2)[:,:,:max_length]
           #alignments_batch = torch.nn.utils.rnn.pad_sequence(alignments_batch, batch_first=True, padding_value=0)[:,:max_length,:]
            ...
            for i in range(0, len(batch_pred_mel), self.vocoder_batch_size):
                ...
                #pred_mel_part_batch = pred_mel_part_batch + 1.0# increase volume cause vocoder is trained on peak normalized
                #pred_mel_part_batch[pred_mel_part_batch < -11.+1.0] = -11.52
