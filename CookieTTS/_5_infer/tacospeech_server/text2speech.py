...
from CookieTTS._2_ttm.TacoSpeech.model import load_model
...
class T2S:
    def __init__(self, conf):
        ...
        assert self.ttm_current in self.conf['TTM']['models'].keys(), "tacospeech default model not found in config models"
        tacospeech_path = self.conf['TTM']['models'][self.ttm_current]['modelpath'] # get first available tacospeech
        self.tacospeech, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup = self.load_tacospeech(tacospeech_path)
    ...
    def load_tacospeech(self, tacospeech_path):
        """Loads tacospeech,
        Returns:
        - model
        - hparams
        - speaker_lookup
        """
        checkpoint = torch.load(tacospeech_path) # load file into memory
        print("Loading tacospeech... ", end="")
        ...
        tacospeech_speaker_name_lookup = checkpoint['speaker_name_lookup'] # save speaker name lookup
        tacospeech_speaker_id_lookup   = checkpoint['speaker_id_lookup']   # save speaker_id lookup
        
        print(f"This tacospeech model has been trained for {checkpoint['iteration']} Iterations.")
        return model, checkpoint_hparams, tacospeech_speaker_name_lookup, tacospeech_speaker_id_lookup
    
    
    def update_mf(self, tacospeech_name):
        self.tacospeech, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup = self.load_tacospeech(self.conf['TTM']['models'][tacospeech_name]['modelpath'])
        self.ttm_current = tacospeech_name
        
        if self.conf['TTM']['models'][tacospeech_name]['use_speaker_ids_file_override']:# (optional) override
            self.ttm_sp_name_lookup = {name.strip(): self.ttm_sp_id_lookup[int(ext_id.strip())] for _, name, ext_id, *_ in load_filepaths_and_text(self.conf['TTM']['models'][tacospeech_name]['speaker_ids_file'])}
    ...
    def infer(self, text, speaker, use_arpabet, cat_silence_s,
        ...
        # update tacospeech stopping params
        frames_per_second = float(self.ttm_hparams.sampling_rate/self.ttm_hparams.hop_length)
        self.tacospeech.max_decoder_steps = int(min(max([len(t) for t in texts]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))
        ...
        for text_index, text in enumerate(texts):
            ...
            self.tacospeech.max_decoder_steps = int(min(max([len(t) for t in text_batch]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))
            ...
            # get speaker_ids (tacospeech)
            tacospeech_speaker_ids = [self.ttm_sp_name_lookup[speaker] for speaker in batch_speaker_names]
            tacospeech_speaker_ids = torch.LongTensor(tacospeech_speaker_ids).cuda().repeat_interleave(batch_size_per_text)
            ...
            style_input = style_input.to(next(self.tacospeech.parameters()).dtype)
            ...
            if status_updates: mf_start=time.time(); print("Running tacospeech", end='')
            try:
                ...
                while np.amin(best_score) < target_score:
                    ...
                    outputs = self.tacospeech.inference(sequence, text_lengths.repeat_interleave(batch_size_per_text, dim=0), tacospeech_speaker_ids, style_input, res_sigma=global_sigma, mel_z_sigma=frame_sigma)
            ...
            assert not any([x==0 for x in best_generations]), 'tacospeech Failed to generate one of the texts after multiple attempts.'
