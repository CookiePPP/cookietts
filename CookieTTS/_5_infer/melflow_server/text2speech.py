...
from CookieTTS._2_ttm.MelFlow.model import load_model
...
class T2S:
    def __init__(self, conf):
        ...
        assert self.ttm_current in self.conf['TTM']['models'].keys(), "MelFlow default model not found in config models"
        melflow_path = self.conf['TTM']['models'][self.ttm_current]['modelpath'] # get first available MelFlow
        self.melflow, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup = self.load_melflow(melflow_path)
    ...
    def load_melflow(self, melflow_path):
        """Loads melflow,
        Returns:
        - model
        - hparams
        - speaker_lookup
        """
        checkpoint = torch.load(melflow_path) # load file into memory
        print("Loading MelFlow... ", end="")
        ...
        melflow_speaker_name_lookup = checkpoint['speaker_name_lookup'] # save speaker name lookup
        melflow_speaker_id_lookup   = checkpoint['speaker_id_lookup']   # save speaker_id lookup
        
        print(f"This MelFlow model has been trained for {checkpoint['iteration']} Iterations.")
        return model, checkpoint_hparams, melflow_speaker_name_lookup, melflow_speaker_id_lookup
    
    
    def update_mf(self, melflow_name):
        self.melflow, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup = self.load_melflow(self.conf['TTM']['models'][melflow_name]['modelpath'])
        self.ttm_current = melflow_name
        
        if self.conf['TTM']['models'][melflow_name]['use_speaker_ids_file_override']:# (optional) override
            self.ttm_sp_name_lookup = {name.strip(): self.ttm_sp_id_lookup[int(ext_id.strip())] for _, name, ext_id, *_ in load_filepaths_and_text(self.conf['TTM']['models'][melflow_name]['speaker_ids_file'])}
    ...
    def infer(self, text, speaker, use_arpabet, cat_silence_s,
              ...
              textseg_len_target, split_nl, split_quo, multispeaker_mode, seed, sigma_scaler,
        ...
        # update MelFlow stopping params
        frames_per_second = float(self.ttm_hparams.sampling_rate/self.ttm_hparams.hop_length)
        self.melflow.max_decoder_steps = int(min(max([len(t) for t in texts]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))
        ...
        for text_index, text in enumerate(texts):
            ...
            self.melflow.max_decoder_steps = int(min(max([len(t) for t in text_batch]) * float(dyna_max_duration_s)*frames_per_second, float(max_duration_s)*frames_per_second))
            ...
            # get speaker_ids (MelFlow)
            melflow_speaker_ids = [self.ttm_sp_name_lookup[speaker] for speaker in batch_speaker_names]
            melflow_speaker_ids = torch.LongTensor(melflow_speaker_ids).cuda().repeat_interleave(batch_size_per_text)
            ...
            style_input = style_input.to(next(self.melflow.parameters()).dtype)
            ...
            if status_updates: mf_start=time.time(); print("Running melflow", end='')
            try:
                ...
                while np.amin(best_score) < target_score:
                    ...
                    outputs = self.melflow.infer(sequence, text_lengths.repeat_interleave(batch_size_per_text, dim=0), melflow_speaker_ids, style_input)
            ...
            assert not any([x==0 for x in best_generations]), 'MelFlow Failed to generate one of the texts after multiple attempts.'
