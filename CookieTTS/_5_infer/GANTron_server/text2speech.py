...
from CookieTTS._2_ttm.GANTron.model import load_model
...
class T2S:
    def __init__(self, conf):
        ...
        assert self.ttm_current in self.conf['TTM']['models'].keys(), "GANTron default model not found in config models"
        GANTron_path = self.conf['TTM']['models'][self.ttm_current]['modelpath'] # get first available GANTron
        self.GANTron, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup, self.ttm_sp_f0_meanstd, self.ttm_sp_sylps_meanstd = self.load_GANTron(GANTron_path)
    ...
    def load_GANTron(self, GANTron_path):
        """Loads GANTron,
        Returns:
        - model
        - hparams
        - speaker_lookup
        """
        checkpoint = torch.load(GANTron_path) # load file into memory
        print("Loading GANTron... ", end="")
        ...
        print(f"This GANTron model has been trained for {checkpoint['iteration']} Iterations.")
        return model, checkpoint_hparams, speaker_name_lookup, speaker_id_lookup, speaker_f0_meanstd, speaker_sylps_meanstd
    
    
    def update_mf(self, GANTron_name):
        self.GANTron, self.ttm_hparams, self.ttm_sp_name_lookup, self.ttm_sp_id_lookup, self.ttm_sp_f0_meanstd, self.ttm_sp_sylps_meanstd = self.load_GANTron(self.conf['TTM']['models'][GANTron_name]['modelpath'])
        self.ttm_current = GANTron_name
        
        if self.conf['TTM']['models'][GANTron_name]['use_speaker_ids_file_override']:# (optional) override
            self.ttm_sp_name_lookup = {name.strip(): self.ttm_sp_id_lookup[int(ext_id.strip())] for _, name, ext_id, *_ in load_filepaths_and_text(self.conf['TTM']['models'][GANTron_name]['speaker_ids_file'])}
    ...
    def infer(self, text, speaker, use_arpabet, cat_silence_s,
              ...
              end_mode='thresh', gate_threshold=0.2,
        ...
        if True:# if force_monotonic_alignment
            forward_range = 2
            self.GANTron.generator.decoder.attention.window_offset = forward_range/2
            self.GANTron.generator.decoder.attention.window_range  = forward_range/2
        # /tmp/
        ...
        # update GANTron stopping params
        ...
        for text_index, text in enumerate(texts):
            ...
            style_input = style_input.to(next(self.GANTron.parameters()).dtype)
            ...
            if status_updates: mf_start=time.time(); print("Running GANTron", end='')
            try:
                ...
                while np.amin(best_score) < target_score:
                    ...
                    outputs = self.GANTron.generator.infer(sequence, text_lengths, speaker_ids, speaker_f0_meanstd, speaker_sylps_meanstd, style_input, sigma=sigma, randn_max=randn_max, max_decode_length=max_decode_length, gate_threshold=gate_threshold)
                    pred_mel_batch   = outputs['hifigan_inputs'] if self.MTW_conf.uses_latent_input else outputs['pred_mel']
                    pred_gate_batch  = outputs['pred_gate'] # [B, mel_T]
                    alignments_batch = outputs['alignments']# [B, mel_T, txt_T]
                    ...
                    if end_mode == 'thresh':
                        mel_lengths = get_first_over_thresh(pred_gate_batch, gate_threshold)
                    elif end_mode == 'max':
                        mel_lengths = pred_gate_batch.argmax(dim=1)
                    ...
                    for j in range(simultaneous_texts): # process each set of text spectrograms seperately
                        ...
                        for k, (mel_length, pred_mel, pred_gate, alignments, diagonality, avg_prob, enc_max_focus, enc_min_focus, enc_avg_focus, p_missing_enc) in enumerate(sametext_batch):
                            ...
                            if weighted_score > best_score[j]:
                                ...
                                best_generations[j] = [pred_mel, mel_length, alignments]
                            else:
            ...
            # [[mel, gate, align], [mel, gate, align], [mel, gate, align]] -> [[mel, mel, mel], [gate, gate, gate], [align, align, align]]
