    ...
    def log_validation(self, reduced_loss_dict, reduced_bestval_loss_dict, model, y, y_pred, iteration,):
        ...
        for idx in range(n_items):# plot target spectrogram of longest audio file(s)
            ...
            self.add_image(f"{prepend}_{idx}/hifi_mel_gt",
                plot_spectrogram_to_numpy(y['gt_mel'][idx, :, :mel_len].data.cpu().numpy(), range=mag_range),
                iteration, dataformats='HWC')
            
            self.add_image(f"{prepend}_{idx}/hifi_mel_pred",
                plot_spectrogram_to_numpy(y_pred['pred_mel'][idx, :, :mel_len].data.cpu().numpy(), range=mag_range),
                iteration, dataformats='HWC')
            
            audio_len = y['audio_lengths'][idx].item()
            self.add_audio(f"{prepend}_{idx}/hifi_audio_gt", y['gt_audio'][idx].data.cpu().squeeze()[:audio_len].unsqueeze(-1),
                iteration, sample_rate=self.sampling_rate)
            
            self.add_audio(f"{prepend}_{idx}/hifi_audio_pred", y_pred['pred_audio'][idx].data.cpu().squeeze()[:audio_len].unsqueeze(-1),
                iteration, sample_rate=self.sampling_rate)
